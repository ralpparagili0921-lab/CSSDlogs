...
create table if not exists app_meta (
  ...
  -- How long acknowledging an alarm silences it before it resumes, if
  -- the underlying task (cycle/BI result) is still unresolved.
  -- Superuser-adjustable in Admin, default 5 min per research into
  -- task-interruption reminder conventions (not a regulated clinical
  -- alarm standard — this isn't an acute physiological alarm).
  alarm_snooze_minutes integer not null default 5,
  -- RO Water Quality's monitoring kit hasn't been purchased/received
  -- yet — while false, the scheduled RO alarm never fires (a superuser
  -- flips this on once the kit actually arrives, via a yellow
  -- "Activate" button at the top of the RO page). Doesn't affect the
  -- ability to log RO entries manually, only the alarm.
  ro_monitoring_activated boolean not null default false,
  ro_monitoring_activated_at timestamptz,  -- when Activate was actually clicked — missed-entries tracking for RO starts from here, not before (no kit = no expectation of daily logging)
  ro_monitoring_activated_by_id uuid references staff(id),  -- Identity Confirm rollout
  ro_monitoring_activated_by_name text,
...
create table if not exists logbook_assignments (
  id uuid primary key default gen_random_uuid(),
  logbook text not null check (logbook in ('ro','equipment','cycles','qa','brush','instrument','handover','temp-humidity-am','temp-humidity-pm','housekeeping')),
...
create table if not exists ro_testers (
  ...
  make text,
  active boolean not null default true,
  added_by_id uuid references staff(id),  -- Identity Confirm rollout
  added_by_name text,
  created_at timestamptz not null default now()
);
...
create table if not exists ro_water_quality (
  ...
  remarks text,
  late_reason text,  -- required, min 15 words, only when logged after that day's 2:30pm deadline — see js/late-entry-reason.js
  staff_id uuid references staff(id),
...
create table if not exists equipment_downtime (
  ...
  staff_id uuid references staff(id),
  staff_name text not null,
  -- Identity Confirm rollout — the pair above only ever captures who
  -- logged the ORIGINAL incident; these capture the two later,
  -- separate recording points that previously had no identity at all.
  biomed_response_confirmed_by_id uuid references staff(id),
  biomed_response_confirmed_by_name text,
...
create table if not exists brush_logs (
  ...
  remarks text,
  late_reason text,  -- required, min 15 words, only when logged after Wednesday 2:30pm (Brush's own weekly-not-daily deadline) — see js/late-entry-reason.js
  staff_id uuid references staff(id),
  staff_name text not null,
  created_at timestamptz not null default now()
);
...
create table if not exists sterilization_cycles (
  ...
  -- Start
  operator_start text,
  operator_start_id uuid references staff(id),  -- Identity Confirm rollout — companion to the text field above
  ...
  stopped_by text,
  stopped_by_id uuid references staff(id),  -- Identity Confirm rollout — companion to the text field above
  stop_remarks text,
  alarm_acknowledged_at timestamptz,  -- when the exposure-complete alarm popup was acknowledged — silences the repeating sound/popup, does NOT clear the card's red glow (that only clears when the cycle is actually completed/stopped)
  alarm_acknowledged_by text,
  operator_end text,
  operator_end_id uuid references staff(id),  -- Identity Confirm rollout — companion to the text field above
  timeline_confirmed_by jsonb,  -- Identity Confirm rollout — {flush:{id,name}, drying:{id,name}, hatch:{id,name}, cooldown:{id,name}}, one shared column since the 4 steps share one function
  ...
  class1_tape_changed text,           -- "Level 1 (Type 1) autoclave indicator tape Color Change" — 'Yes' / 'No' / 'Cannot Be Determined'
  class1_tape_failed_packs text,      -- which specific packs did NOT color change — filled in when the above is No or Cannot Be Determined
...
create table if not exists sterilizer_qa_tests (
  ...
  late_reason text,  -- required, min 15 words, only when logged after that test type's own deadline (BI 8:30am, Dummy/Bowie-Dick 8:00am) — see js/late-entry-reason.js
  ...
  bi_prelim_read_by_id uuid references staff(id),  -- Identity Confirm rollout
  bi_prelim_read_by_name text,
  ...
  bi_read_by_id uuid references staff(id),  -- Identity Confirm rollout — who confirmed the RESULT, distinct from staff_id/staff_name above (which is whoever ran Initiate — often a different person, given incubation takes hours)
  bi_read_by_name text,
  alarm_acknowledged_at timestamptz,  -- when the incubation-complete alarm popup was acknowledged — silences the repeating sound/popup, does NOT clear the card's red glow (that only clears once the result is actually logged)
  alarm_acknowledged_by text,
...
create table if not exists housekeeping_logs (
  ...
  inspected_by text,
  late_reason text,  -- required, min 15 words, only when logged after 3:30pm — see js/late-entry-reason.js
  staff_id uuid references staff(id),
  staff_name text not null,
  created_at timestamptz not null default now()
);
...
create table if not exists temp_humidity_logs (
  ...
  remarks text,
  late_reason text,  -- required, min 15 words, only when logged after that reading's own AM (8am) or PM (4pm) deadline — see js/late-entry-reason.js
...
create table if not exists instrument_handovers (
  ...
  -- Intake verification — CSSD physically confirming what was actually
  -- received against what the department claimed to submit, BEFORE
  -- processing/release even begins. Deliberately separate from
  -- received_at (submission) below, which stays the actual submission
  -- moment the 3-day turnaround clock is measured against — CSSD's own
  -- delay in verifying still counts against their own target, it isn't
  -- an excuse to push the clock back. Per-item intake_qty/intake_remarks
  -- live inside load_contents above, same pattern as received_qty.
  intake_verified_at timestamptz,
  intake_verified_by_id uuid references staff(id),
  intake_verified_by_name text,
  has_intake_discrepancy boolean not null default false,  -- true if any item's counted intake_qty < submitted qty
  intake_discrepancy_notified_at timestamptz,   -- when CSSD confirmed they told the department about the shortfall
  intake_discrepancy_notified_by_id uuid references staff(id),  -- Identity Confirm rollout — companion to the name field below
  intake_discrepancy_notified_by_name text,
...
-- ---------- HANDOVER ITEMS (per-item-line tracking, dual-PIN confirmed) ----------
-- instrument_handovers above stays the "batch" record (reference
-- number, department, shared context) — each distinct item line
-- submitted against that reference gets its own row here, so it's
-- independently traceable rather than buried inside a JSONB array.
...
-- item — no "pending" or half-confirmed row exists by design. Both
-- confirmation columns reference `staff` because department accounts
-- and CSSD staff are the same table, distinguished by staff.role —
-- see DB.listDepartmentAccounts().
create table if not exists handover_items (
  ...
  -- Release pathway — CSSD marks an item ready once sterilization is
  -- actually done; only then do the two Release PIN buttons appear on
  -- the department portal. Order matters: department confirms first
  -- (recording any discrepancy live as they enter it), then CSSD's own
  -- PIN step — shown the discrepancy, if any — is their affirmation of
  -- it, not a separate action.
  ready_for_release_by_id uuid references staff(id),
  ready_for_release_by_name text,
  ready_for_release_at timestamptz,
  dept_release_confirmed_by_id uuid references staff(id),
  dept_release_confirmed_by_name text,
  dept_release_confirmed_at timestamptz,
  cssd_release_confirmed_by_id uuid references staff(id),
  cssd_release_confirmed_by_name text,
  cssd_release_confirmed_at timestamptz,
  has_discrepancy boolean not null default false,
  received_qty integer,  -- only set if a discrepancy was recorded; null implies "matched qty exactly"
  created_at timestamptz not null default now()
);
...
-- ---------- ERROR REPORTS ----------
-- Captures both truly-uncaught JS errors (window.onerror /
-- unhandledrejection) and explicit user reports from the "Report to
-- Superuser" popup shown when something breaks. staff_id is nullable
-- since an error can happen before login. status lets a superuser
-- triage without needing to fix everything the moment it's reported.
...
-- ---------- SERVER TIME (authoritative clock source) ----------
-- Lets the client compute the offset between a device's own clock
-- (which can drift or be misconfigured — a real risk for a
-- compliance-critical log) and this server's actual clock, then
-- correct every captured timestamp by that offset. Postgres's own
-- `now()` is already reliable and internet-time-synced at the OS
-- level — no external time API dependency needed.
