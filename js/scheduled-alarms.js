...
const ScheduledAlarms = {
  ...
  async tick() {
    // None of these are the department portal's business — every check
    // below is a CSSD-internal logbook or turnaround concern, and the
    // alarm panel itself is a global document.body overlay with no
    // per-view scoping, so without this check any CSSD alarm would
    // sound and show even while a department account is logged into
    // their own portal on a totally different device. Checked by
    // element, not by who's logged in — a shared CSSD device should
    // still hear these even pre-login; a department account's own
    // portal should never hear them regardless of its own login state.
    const deptPortalEl = document.getElementById('view-handover-submit');
    if (deptPortalEl && !deptPortalEl.classList.contains('hidden')) {
      ['cycle-exposure-', 'bi-incubation-', 'sched-handover-', 'sched-bi-', 'sched-dummy-', 'sched-ro', 'sched-brush-', 'sched-th-am-', 'sched-th-pm-', 'sched-hk']
        .forEach(prefix => this._clearAllWithPrefix(prefix));
      return;
    }
    ...
    await this._checkHandoverItemTurnaround();
  ...
  async _checkHandoverTurnaround() {
    try {
      // Excludes batches from the new per-item flow — see
      // _checkHandoverItemTurnaround below, which tracks each of
      // THEIR items individually and accurately. Without this
      // exclusion, a new-style batch would alarm here too (its batch
      // record also gets status:'Processing'), AND its per-item
      // clock would be wrong for any batch with more than one item —
      // every item in it would share the batch's single received_at,
      // even though each was actually confirmed at its own, possibly
      // much later, moment.
      const itemHandoverIds = await DB.listHandoverIdsWithItems();
      const processing = (await DB.listProcessingHandovers()).filter(r => !itemHandoverIds.has(r.id));
  ...
  // Per-item counterpart to the check above, for batches from the new
  // per-item flow — each item's own clock starts from when ITS two
  // Submission PINs actually landed, not from the shared batch-level
  // received_at, since items in the same batch can genuinely be
  // confirmed hours or days apart from each other.
  async _checkHandoverItemTurnaround() {
    try {
      const items = await DB.listHandoverItemsInProgress();
      const batches = await DB.listProcessingHandovers();
      const batchById = new Map(batches.map(b => [b.id, b]));
      const late = items.filter(it => {
        const batch = batchById.get(it.handover_id);
        if (!batch) return false; // batch not 'Processing' (or gone) — nothing to alarm about
        const submittedAt = Math.max(new Date(it.dept_submit_confirmed_at).getTime(), new Date(it.cssd_submit_confirmed_at).getTime());
        return TrueTime.now().getTime() - submittedAt > 3 * 86400000;
      });
      const currentKeys = new Set(late.map(it => `sched-handover-item-${it.id}`));

      // Same explicit-resolve need as the batch-level check — an item
      // that stops appearing here (released, or its batch left
      // Processing) never reappears in this query once that happens.
      (this._lastLateHandoverItemKeys || new Set()).forEach(key => {
        if (!currentKeys.has(key)) { this._resolve(key); }
      });
      this._lastLateHandoverItemKeys = currentKeys;
      ...
      late.forEach(it => {
        const batch = batchById.get(it.handover_id);
        const key = `sched-handover-item-${it.id}`;
        const submittedAt = Math.max(new Date(it.dept_submit_confirmed_at).getTime(), new Date(it.cssd_submit_confirmed_at).getTime());
        const overdueMs = TrueTime.now().getTime() - (submittedAt + 3 * 86400000);
        const isVeryLate = overdueMs >= 1.5 * 86400000;
        this._fireOrResolve(
          key, false, primaryName, isVeryLate ? secondaryName : null,
          'Handover Alert',
          `${HandoverView._deptLabel(batch)} — ${it.item_name} is past the 3-day release target`,
          'Handover Item Past Turnaround Target',
          `${HandoverView._deptLabel(batch)} · ${batch.serial_number} · ${it.item_name} — submitted ${UI.fmtDate(new Date(submittedAt).toISOString())}`
        );
      });
    } catch (e) { /* offline or unreachable — try again next tick */ }
  },
  ...
  _clearAllWithPrefix(prefix) {
    Alarm.activeKeys().forEach(k => { if (k.startsWith(prefix)) { Alarm.stop(k); Alarm.removeBox(k); } });
  }
};
