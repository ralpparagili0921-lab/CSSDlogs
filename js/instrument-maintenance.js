...
const InstrumentMaintenanceView = {
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="section-title" style="display:flex;align-items:center;justify-content:space-between;">
        <span>Recent entries <span class="count" id="im-count">—</span></span>
        <button class="btn btn-sm" id="im-view-history">View all history →</button>
      </div>
      ${SearchBar.render('im-search')}
      <div class="table-wrap"><table>
        <thead><tr><th>Ref #</th><th>Sent out</th><th>Reason</th><th>Instrument / Set</th><th>Status</th><th>Returned</th><th>Logged by</th><th></th></tr></thead>
        <tbody id="im-tbody"><tr><td colspan="8" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
    ...
    document.getElementById('im-view-history').addEventListener('click', () => App.navigate('im-history'));
    this._loadOpen();
    this._loadTable();
  },
  ...
  _openReturnModal(row) {
    ...
    modal.querySelector('#im-return-confirm').addEventListener('click', async () => {
      // Read the form value and close THIS modal before opening
      // IdentityConfirm's own — see recurring bug pattern #17: two
      // simultaneously-open modals share the same active-modal id,
      // so IdentityConfirm closing itself would actually close this
      // one instead, leaving its own stuck open.
      const returnNotes = modal.querySelector('#im-return-notes').value || null;
      UI.closeModal();
      const who = await IdentityConfirm.confirm('Mark Returned');
      if (!who) return;
      try {
        const result = await DB.updateInstrumentMaintenance(row.id, {
          status: 'Returned',
          returned_at: TrueTime.nowISO(),
          returned_by_id: who.id,
  ...
  async _submit(e) {
    ...
    const isSet = entryMode === 'set';
    const who = await IdentityConfirm.confirm('Send Out');
    if (!who) return;
    const entry = {
  ...
  _renderTable(rows, tbodyId = 'im-tbody', countId = 'im-count') {
    ...
    tbody.innerHTML = rows.map(r => `
      ...
      <tr id="im-items-row-${r.id}" style="display:none;"><td colspan="8" style="padding:12px 16px;white-space:pre-line;background:var(--surface-sunken);">${UI.escapeHtml(this._label(r)) || '(no items listed)'}</td></tr>
    `).join('');
  ...
  _printEntry(r) {
    const rows = [
      ['Sent out', UI.fmtDateTime(r.created_at)], ['Reason', r.action_out],
      ['Entry mode', r.entry_mode === 'set' ? 'Set/Tray' : 'Individual'],
      ['Item(s)', this._label(r)],
      ['From tray/case', r.from_tray_set ? 'Yes' : 'No'],
      ['Tray/case number', r.tray_case_number],
      ['Status', r.status],
      ['Returned', r.returned_at ? `${UI.fmtDateTime(r.returned_at)} by ${r.returned_by_name || '—'}` : '—'],
      ['Logged by', r.staff_name]
    ];
    PrintReport.generate({ title: 'INSTRUMENT MAINTENANCE LOG REPORT', refNumber: r.serial_number, sections: [{ heading: 'Entry Details', rows }] });
  }
};

const InstrumentMaintenanceHistoryView = {
  async render() {
    const container = document.getElementById('view-im-history');
    const { tableWrap, setCount } = HistoryView.renderShell({
      container, title: 'Instrument Maintenance Log — Full History', backView: 'instruments',
    ...
    tableWrap.innerHTML = `
      <div class="table-wrap">
        <table>
          <thead><tr><th>Ref #</th><th>Sent out</th><th>Reason</th><th>Instrument / Set</th><th>Status</th><th>Returned</th><th>Logged by</th><th></th></tr></thead>
          <tbody id="im-hist-tbody"><tr><td colspan="8" class="empty-state">Loading…</td></tr></tbody>
        </table>
      </div>
    `;
  },
  ...
  async _load(filters, setCount) {
    const tbody = document.getElementById('im-hist-tbody');
    try {
      const rows = await DB.listInstrumentMaintenance({ from: filters.from, to: filters.to, limit: 1000 });
      const filtered = filters.search ? SearchBar.filter(rows, { text: filters.search }, 'created_at', ['action_out', 'instrument_name', 'set_tray_name', 'staff_name']) : rows;
      setCount(filtered.length);
      InstrumentMaintenanceView._renderTable.call(InstrumentMaintenanceView, filtered, 'im-hist-tbody', 'hv-count');
    } catch (e) {
