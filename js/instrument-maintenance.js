...
const InstrumentMaintenanceView = {
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Sent out</th><th>Reason</th><th>Instrument / Set</th><th>Status</th><th>Returned</th><th>Logged by</th></tr></thead>
        <tbody id="im-tbody"><tr><td colspan="6" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
  ...
  async _loadTable() {
    ...
    try {
      ...
      document.getElementById('im-count').textContent = `${rows.length} in last 30 days`;
      if (rows.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="empty-state">No entries yet in the last 30 days.</td></tr>`;
        return;
      }
      tbody.innerHTML = rows.map(r => `
        <tr>
          ...
          <td><strong><span style="white-space:pre-line;">${UI.escapeHtml(this._label(r))}</span></strong></td>
      ...
      tbody.innerHTML = `<tr><td colspan="6" class="empty-state">Couldn't load entries: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  }
};
