    ...
    try {
      ...
      if (balanceRows.length === 0) {
        ...
        bWrap.innerHTML = balanceRows.map(r => {
          const short = (r.load_contents || []).filter(it => (it.received_qty ?? it.qty) < it.qty);
          return `
            <div class="card card-pad pending-highlight" style="margin-bottom:10px;">
              ...
              ${short.map(it => `
                <div style="margin-bottom:6px;">
                  ...
                  ${it.final_status ? `<span class="badge ${it.final_status === 'final_resolved' ? 'badge-pass' : 'badge-fail'}" style="margin-left:6px;">${it.final_status === 'final_resolved' ? 'Final Resolved' : 'Final Unresolved'}</span>`
                    : it.cssd_action ? `<span class="badge ${it.cssd_action === 'resolved' ? 'badge-pass' : 'badge-open'}" style="margin-left:6px;">CSSD: ${it.cssd_action === 'resolved' ? 'Resolved' : 'Unresolved — awaiting final review'}</span>`
