...
const DepartmentPortal = {
  ...
  _openVerifyRemainingModal(handoverId, itemId) {
    const modal = UI.showModal(`
      ...
      <div class="field" style="margin-bottom:14px;">
        ...
        <div class="pin-dots" id="vr-dots"><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div></div>
        <div class="pin-pad" id="vr-pad">
    ...
    (async () => {
      try {
        ...
        let buf = '';
        const dots = () => modal.querySelectorAll('#vr-dots .pin-dot').forEach((d, i) => d.classList.toggle('filled', i < buf.length));
        modal.querySelectorAll('#vr-pad .pin-key').forEach(k => k.addEventListener('click', async () => {
          const key = k.dataset.key;
          if (key === 'clear') { buf = ''; dots(); return; }
          if (key === 'back') { buf = buf.slice(0, -1); dots(); return; }
          if (buf.length >= 4) return;
          ...
          if (buf.length === 4) {
            ...
            try {
              ...
              if (!me || me.pin !== entered) {
                document.getElementById('vr-error').textContent = 'Incorrect PIN — try again.';
                dots();
                return;
              }
              ...
              const updatedItems = handover.load_contents.map(it => it.id !== itemId ? it : {
                ...
                reverify_qty: null,
                remaining_verified_by_id: me.id,
                remaining_verified_by_name: me.name,
              ...
              document.getElementById('vr-error').textContent = 'Could not save: ' + e.message;
              dots();
            }
          }
        }));
  ...
  _renderSubmit() {
    ...
    this._shell(`
      ...
      <div class="card card-pad" id="hsp-add-item-card">
        <div class="form-grid" style="grid-template-columns:2fr 1fr auto;gap:8px;align-items:end;">
          <div class="field"><label>Item</label><input type="text" id="hsp-new-item-name"></div>
          <div class="field"><label>Qty</label><input type="number" id="hsp-new-item-qty" min="1" value="1"></div>
          <button type="button" class="btn btn-primary" id="hsp-add-item">+ Add</button>
        </div>
      </div>

      <div class="section-title">Verification <span class="hint" style="font-weight:400;">both sides confirm with their own PIN — nothing is submitted until both are entered</span></div>
      <div id="hsp-pending-list"></div>
      <div style="font-size:11.5px;color:var(--ink-soft);margin-top:16px;text-align:center;">The date and time for each item are recorded automatically the moment it's submitted.</div>
    `);
    this._wireSessionBar();
    document.getElementById('hsp-add-item').addEventListener('click', () => this._addPendingItem());
    this._loadIntakeDiscrepancies();
    this._renderPendingItems();

    // Items awaiting confirmation are pure client-side state until
    // both PINs land — nothing exists in the DB for them yet, so if
    // the page gets refreshed while waiting on a second PIN, this is
    // the only thing standing between that and losing the whole list.
    // Deliberately does NOT persist which PINs were already entered
    // (see _renderPendingItems' comment) — only the item name/qty.
    FormDraft.attach(document.getElementById('hsp-add-item-card'), 'hsp-pending-items', {
      getExtra: () => ({ items: this._pendingItems.map(it => ({ name: it.name, qty: it.qty })) }),
      setExtra: (extra) => {
        if (!extra || !extra.items || extra.items.length === 0) return;
        this._pendingItems = extra.items.map(it => ({ tempId: generateUUID(), name: it.name, qty: it.qty, deptConfirmed: null, cssdConfirmed: null }));
        this._renderPendingItems();
      }
    });
  },
  ...
  _addPendingItem() {
    const nameInput = document.getElementById('hsp-new-item-name');
    const qtyInput = document.getElementById('hsp-new-item-qty');
    const name = nameInput.value.trim();
    const qty = parseInt(qtyInput.value, 10) || 0;
    if (!name) { UI.toast('Enter an item name', true); return; }
    if (qty < 1) { UI.toast('Quantity must be at least 1', true); return; }
    this._pendingItems.push({ tempId: generateUUID(), name, qty, deptConfirmed: null, cssdConfirmed: null });
    nameInput.value = ''; qtyInput.value = '1';
    nameInput.focus();
    this._renderPendingItems();
  },

  // Renders every item still awaiting confirmation, submission, or
  // both. Deliberately does not use FormDraft's getExtra/setExtra for
  // the deptConfirmed/cssdConfirmed flags themselves — a PIN
  // confirmation should never silently survive a page refresh; if
  // the page reloads mid-confirmation, both sides re-enter their PIN,
  // which is the safer default given the whole point of this flow is
  // that someone genuinely, physically confirmed it.
  _renderPendingItems() {
    const wrap = document.getElementById('hsp-pending-list');
    if (!wrap) return;
    if (this._pendingItems.length === 0) {
      wrap.innerHTML = `<div class="card card-pad empty-state">No items added yet.</div>`;
      return;
    }
    wrap.innerHTML = this._pendingItems.map(it => {
      const bothConfirmed = it.deptConfirmed && it.cssdConfirmed;
      return `
      <div class="card card-pad" style="margin-bottom:10px;">
        <div style="display:flex;justify-content:space-between;align-items:baseline;">
          <strong>${UI.escapeHtml(it.name)}</strong>
          <span class="hint">× ${it.qty}</span>
        </div>
        <div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap;">
          ${it.deptConfirmed
            ? `<span class="badge badge-pass">✓ Pincode Confirmed — Dept: ${UI.escapeHtml(it.deptConfirmed.name)}</span>`
            : `<button class="btn btn-sm" data-dept-pin="${it.tempId}">Enter PIN (Department)</button>`}
          ${it.cssdConfirmed
            ? `<span class="badge badge-pass">✓ Pincode Confirmed — CSSD: ${UI.escapeHtml(it.cssdConfirmed.name)}</span>`
            : `<button class="btn btn-sm" data-cssd-pin="${it.tempId}">Enter PIN (CSSD Staff)</button>`}
        </div>
        ${bothConfirmed ? `<div style="margin-top:10px;"><button class="btn btn-primary btn-sm" data-submit-item="${it.tempId}">Submit</button></div>` : `<div class="hint" style="margin-top:8px;">The Submit button appears once both sides have entered their PIN.</div>`}
      </div>`;
    }).join('');
    wrap.querySelectorAll('[data-dept-pin]').forEach(b => b.addEventListener('click', () => this._openDeptPinModal(b.dataset.deptPin)));
    wrap.querySelectorAll('[data-cssd-pin]').forEach(b => b.addEventListener('click', () => this._openCssdPinModal(b.dataset.cssdPin)));
    wrap.querySelectorAll('[data-submit-item]').forEach(b => b.addEventListener('click', () => this._submitPendingItem(b.dataset.submitItem)));
  },

  // Shared PIN-pad renderer — onMatch(pinBuffer) is called once 4
  // digits are entered; caller decides whether it was actually
  // correct (this doesn't know anyone's PIN itself, just collects
  // the 4 digits) and closes the modal / shows an error accordingly.
  _renderPinPad(label, onComplete) {
    const modal = UI.showModal(`
      <div class="pin-label" style="margin-bottom:10px;">${label}</div>
      <div class="default-pin-hint">Entering your PIN confirms this item is being handed over right now, in person. The Submit button only appears once BOTH the department and CSSD sides have confirmed.</div>
      <div class="pin-dots" id="hsp-modal-pin-dots"><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div></div>
      <div class="pin-pad" id="hsp-modal-pin-pad">
        ${[1,2,3,4,5,6,7,8,9].map(n => `<button class="pin-key" data-key="${n}">${n}</button>`).join('')}
        <button class="pin-key func" data-key="back">←</button>
        <button class="pin-key" data-key="0">0</button>
        <button class="pin-key func" data-key="clear">C</button>
      </div>
      <div id="hsp-modal-pin-error" class="hint" style="color:var(--red);min-height:16px;margin-top:8px;text-align:center;"></div>
      <div style="text-align:center;margin-top:8px;"><button class="btn btn-sm" id="hsp-modal-pin-cancel">Cancel</button></div>
    `);
    modal.querySelector('#hsp-modal-pin-cancel').addEventListener('click', () => UI.closeModal());
    let buf = '';
    const dots = () => modal.querySelectorAll('#hsp-modal-pin-dots .pin-dot').forEach((d, i) => d.classList.toggle('filled', i < buf.length));
    modal.querySelectorAll('#hsp-modal-pin-pad .pin-key').forEach(k => k.addEventListener('click', () => {
      const key = k.dataset.key;
      if (key === 'clear') { buf = ''; dots(); return; }
      if (key === 'back') { buf = buf.slice(0, -1); dots(); return; }
      if (buf.length >= 4) return;
      ...
      if (buf.length === 4) {
        const entered = buf;
        buf = '';
        onComplete(entered, () => { dots(); document.getElementById('hsp-modal-pin-error').textContent = 'Incorrect PIN — try again.'; });
      }
    }));
  },

  // Department staffer re-confirming their OWN PIN — they're already
  // logged in, so no name-selection step, just re-enter their PIN at
  // the moment of this specific accountable action.
  async _openDeptPinModal(tempId) {
    let account;
    try {
      const accounts = await DB.listDepartmentAccounts(this._session.department);
      account = accounts.find(a => a.id === this._session.id);
    } catch (e) { UI.toast('Could not verify — offline or unreachable', true); return; }
    if (!account) { UI.toast('Could not find your account', true); return; }
    this._renderPinPad(`Enter your PIN, ${UI.escapeHtml(account.name)}`, (entered, onWrong) => {
      if (entered === account.pin) {
        UI.closeModal();
        const it = this._pendingItems.find(i => i.tempId === tempId);
        if (it) { it.deptConfirmed = { id: account.id, name: account.name }; this._renderPendingItems(); }
      } else { onWrong(); }
    });
  },

  // CSSD staffer confirming — nobody from CSSD is logged into this
  // session at all, so this needs its own name-selection step first.
  async _openCssdPinModal(tempId) {
    let staff;
    try { staff = await DB.listActiveStaff(); } catch (e) { UI.toast('Could not load staff — offline or unreachable', true); return; }
    const modal = UI.showModal(`
      <div class="pin-label" style="margin-bottom:8px;">Which CSSD staff member?</div>
      <div class="staff-grid" id="hsp-cssd-name-grid">
        ${staff.map(s => `<button class="staff-btn" data-id="${s.id}">${UI.escapeHtml(s.name)}</button>`).join('')}
      </div>
      <div style="text-align:center;margin-top:12px;"><button class="btn btn-sm" id="hsp-cssd-submit-cancel">Cancel</button></div>
    `);
    modal.querySelector('#hsp-cssd-submit-cancel').addEventListener('click', () => UI.closeModal());
    modal.querySelectorAll('[data-id]').forEach(btn => btn.addEventListener('click', () => {
      const s = staff.find(x => x.id === btn.dataset.id);
      UI.closeModal();
      this._renderPinPad(`Enter PIN for ${UI.escapeHtml(s.name)}`, (entered, onWrong) => {
        if (entered === s.pin) {
          UI.closeModal();
          const it = this._pendingItems.find(i => i.tempId === tempId);
          if (it) { it.cssdConfirmed = { id: s.id, name: s.name }; this._renderPendingItems(); }
        } else { onWrong(); }
      });
    }));
  },

  // Only ever called once both PINs are confirmed for this specific
  // item — this is the single moment anything for this item actually
  // gets written to the database. Lazily creates the batch record on
  // the first item of a session; every later item in the same
  // session reuses that same handover_id.
  async _submitPendingItem(tempId) {
    const it = this._pendingItems.find(i => i.tempId === tempId);
    if (!it || !it.deptConfirmed || !it.cssdConfirmed) return;
    try {
      if (!this._currentHandoverId) {
        const newHandoverId = generateUUID();
        await DB.addHandoverIntake({
          id: newHandoverId,
        ...
        // Only commit to instance state once the insert has actually
        // succeeded (or been safely queued for offline sync) — if it
        // threw, this line never runs, so a retry correctly attempts
        // to create the batch record again instead of being
        // permanently stuck referencing a batch that was never made.
        this._currentHandoverId = newHandoverId;
      }
      const itemPayload = {
        item_name: it.name, qty: it.qty,
        dept_submit_confirmed_by_id: it.deptConfirmed.id, dept_submit_confirmed_by_name: it.deptConfirmed.name, dept_submit_confirmed_at: TrueTime.nowISO(),
        cssd_submit_confirmed_by_id: it.cssdConfirmed.id, cssd_submit_confirmed_by_name: it.cssdConfirmed.name, cssd_submit_confirmed_at: TrueTime.nowISO()
      };
      const result = await DB.addHandoverItem(this._currentHandoverId, itemPayload);
      // Keeping load_contents in sync is a convenience for the existing
      // wide-dashboard/status-badge code, not the source of truth for
      // this new flow — its own read isn't offline-queueable, so a
      // failure here is logged but never blocks the real submission,
      // which already succeeded via handover_items above.
      try { await DB.appendHandoverLoadContentsItem(this._currentHandoverId, it); } catch (e) { /* non-blocking */ }
      UI.writeResultToast(result, `${it.name} submitted`);
      this._pendingItems = this._pendingItems.filter(i => i.tempId !== tempId);
      this._renderPendingItems();
    } catch (e) { UI.toast('Could not submit: ' + e.message, true); }
  },

  async _loadItemsReadyForRelease() {
    const wrap = document.getElementById('dp-items-release-list');
    if (!wrap) return;
    try {
      const rows = await DB.listHandoverItemsReadyForRelease(this._session.department);
      this._releaseItems = rows;
      if (rows.length === 0) {
        wrap.innerHTML = `<div class="card card-pad empty-state" style="padding:18px;">Nothing ready for release right now.</div>`;
        return;
      }
      wrap.innerHTML = rows.map(r => `
        <div class="card card-pad" style="margin-bottom:10px;">
          <div style="display:flex;justify-content:space-between;align-items:baseline;">
            <strong>${UI.escapeHtml(r.item_name)}</strong>
            <span class="hint">× ${r.qty}</span>
          </div>
          <div class="hint" style="margin:4px 0 10px;">Marked ready ${UI.fmtDateTime(r.ready_for_release_at)} by ${UI.escapeHtml(r.ready_for_release_by_name) || 'CSSD'}</div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            ${r.dept_release_confirmed_at
              ? `<span class="badge badge-pass">✓ Pincode Confirmed — Dept: ${UI.escapeHtml(r.dept_release_confirmed_by_name)}</span>`
              : `<button class="btn btn-sm" data-dept-release-pin="${r.id}">Enter PIN (Department)</button>`}
            ${r.dept_release_confirmed_at
              ? `<button class="btn btn-sm" data-cssd-release-pin="${r.id}">Enter PIN (CSSD Staff)</button>`
              : `<button class="btn btn-sm" disabled title="Department confirms first">Enter PIN (CSSD Staff)</button>`}
          </div>
          ${!r.dept_release_confirmed_at ? `<div class="hint" style="margin-top:8px;">CSSD's PIN becomes available once the department confirms first.</div>` : ''}
        </div>
      `).join('');
      wrap.querySelectorAll('[data-dept-release-pin]').forEach(b => b.addEventListener('click', () => this._openDeptReleasePinModal(b.dataset.deptReleasePin)));
      wrap.querySelectorAll('[data-cssd-release-pin]:not([disabled])').forEach(b => b.addEventListener('click', () => this._openCssdReleasePinModal(b.dataset.cssdReleasePin)));
    } catch (e) {
      wrap.innerHTML = `<div class="card card-pad empty-state">Couldn't load — offline or unreachable.</div>`;
    }
  },

  // Once an item is fully released, it disappears from the list
  // above with no further visibility — this is the only place that
  // history is shown, since the data itself is saved either way.
  async _loadRecentlyReleasedItems() {
    const wrap = document.getElementById('dp-items-recent-release-list');
    if (!wrap) return;
    try {
      const rows = await DB.listRecentlyReleasedItemsForDepartment(this._session.department);
      if (rows.length === 0) {
        wrap.innerHTML = `<div class="card card-pad empty-state" style="padding:18px;">Nothing released yet.</div>`;
        return;
      }
      wrap.innerHTML = rows.map(r => `
        <div class="card card-pad" style="margin-bottom:10px;">
          ...
          ${r.has_discrepancy
            ? `<div class="hint" style="color:var(--red);margin:4px 0;">Discrepancy — received ${r.received_qty} of ${r.qty}</div>`
            : ''}
          <div class="hint" style="margin-top:6px;">
            Dept confirmed by ${UI.escapeHtml(r.dept_release_confirmed_by_name)} · CSSD confirmed by ${UI.escapeHtml(r.cssd_release_confirmed_by_name)}
            <br>${UI.fmtDateTime(r.cssd_release_confirmed_at)}
          </div>
        </div>
      `).join('');
    } catch (e) {
  ...
  // discrepancy gets recorded, live, before the PIN is even entered.
  // Order matters: this always happens BEFORE CSSD's own release PIN
  // step, since CSSD needs to actually see what's recorded here.
  async _openDeptReleasePinModal(itemId) {
    const item = this._releaseItems.find(r => r.id === itemId);
    if (!item) return;
    let account;
    try {
      const accounts = await DB.listDepartmentAccounts(this._session.department);
    ...
    const modal = UI.showModal(`
      <div class="pin-label" style="margin-bottom:10px;">${UI.escapeHtml(item.item_name)} — verifying receipt</div>
      <div style="display:flex;gap:16px;flex-wrap:wrap;">
        <div style="flex:1;min-width:160px;">
          <div class="hint" style="margin-bottom:8px;">Is there a discrepancy?</div>
          <button class="btn btn-sm" id="dp-rel-disc-yes">Yes</button>
          <div id="dp-rel-disc-detail" style="display:none;margin-top:12px;">
            <div class="hint">Submitted: ${item.qty}</div>
            <label class="hint" style="display:block;margin-top:6px;">Actually received</label>
            <input type="number" id="dp-rel-received-qty" min="0" value="${item.qty}" style="width:80px;">
            <div id="dp-rel-disc-live" class="hint" style="margin-top:6px;"></div>
          </div>
        </div>
        <div style="flex:1;min-width:180px;">
          <div class="default-pin-hint">Entering your PIN confirms this item was physically received right now.</div>
          <div class="pin-dots" id="hsp-modal-pin-dots"><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div><div class="pin-dot"></div></div>
          <div class="pin-pad" id="hsp-modal-pin-pad">
            ${[1,2,3,4,5,6,7,8,9].map(n => `<button class="pin-key" data-key="${n}">${n}</button>`).join('')}
            <button class="pin-key func" data-key="back">←</button>
      ...
      <div style="text-align:center;margin-top:12px;"><button class="btn btn-sm" id="dp-rel-cancel">Cancel</button></div>
    `);
    modal.querySelector('#dp-rel-cancel').addEventListener('click', () => UI.closeModal());
    modal.querySelector('#dp-rel-disc-yes').addEventListener('click', () => {
      hasDiscrepancy = true;
      modal.querySelector('#dp-rel-disc-detail').style.display = '';
      modal.querySelector('#dp-rel-disc-yes').classList.add('btn-primary');
    });
    const receivedInput = modal.querySelector('#dp-rel-received-qty');
    const liveHint = modal.querySelector('#dp-rel-disc-live');
    receivedInput.addEventListener('input', () => {
      const n = parseInt(receivedInput.value, 10);
      liveHint.textContent = isNaN(n) ? '' : (n < item.qty ? `Short by ${item.qty - n}` : n > item.qty ? `${n - item.qty} more than submitted` : 'Matches submitted quantity');
    });
    ...
    modal.querySelectorAll('#hsp-modal-pin-pad .pin-key').forEach(k => k.addEventListener('click', () => {
      ...
      if (buf.length === 4) {
        ...
        if (entered !== account.pin) {
          dots();
          modal.querySelector('#hsp-modal-pin-error').textContent = 'Incorrect PIN — try again.';
          return;
        }
        const receivedQty = hasDiscrepancy ? (parseInt(receivedInput.value, 10) || 0) : item.qty;
        const actuallyDiscrepant = hasDiscrepancy && receivedQty !== item.qty;
        UI.closeModal();
        DB.confirmHandoverItemRelease(itemId, {
          dept_release_confirmed_by_id: account.id, dept_release_confirmed_by_name: account.name, dept_release_confirmed_at: TrueTime.nowISO(),
          has_discrepancy: actuallyDiscrepant, received_qty: actuallyDiscrepant ? receivedQty : null
        }).then((result) => {
          UI.writeResultToast(result, 'Receipt confirmed');
          this._loadItemsReadyForRelease();
        }).catch(err => UI.toast('Could not confirm: ' + err.message, true));
      }
    }));
  },

  // CSSD's release confirmation — always the second step. Shown the
  // discrepancy the department already recorded, if any; entering the
  // PIN here is CSSD's own affirmation of it, not a separate action.
  async _openCssdReleasePinModal(itemId) {
    const item = this._releaseItems.find(r => r.id === itemId);
    if (!item) return;
    let staff;
    try { staff = await DB.listActiveStaff(); } catch (e) { UI.toast('Could not load staff — offline or unreachable', true); return; }
    const modal = UI.showModal(`
      <div class="pin-label" style="margin-bottom:8px;">Which CSSD staff member?</div>
      ${item.has_discrepancy ? `<div class="hint" style="color:var(--red);margin-bottom:10px;">Department recorded a discrepancy — received ${item.received_qty} of ${item.qty} submitted.</div>` : ''}
      <div class="staff-grid" id="hsp-cssd-name-grid">
        ${staff.map(s => `<button class="staff-btn" data-id="${s.id}">${UI.escapeHtml(s.name)}</button>`).join('')}
      </div>
      <div style="text-align:center;margin-top:12px;"><button class="btn btn-sm" id="hsp-cssd-release-cancel">Cancel</button></div>
    `);
    modal.querySelector('#hsp-cssd-release-cancel').addEventListener('click', () => UI.closeModal());
    modal.querySelectorAll('[data-id]').forEach(btn => btn.addEventListener('click', () => {
      const s = staff.find(x => x.id === btn.dataset.id);
      UI.closeModal();
      this._renderPinPad(
        `Enter PIN for ${UI.escapeHtml(s.name)}${item.has_discrepancy ? ' — affirming the discrepancy above' : ''}`,
        (entered, onWrong) => {
          if (entered === s.pin) {
            UI.closeModal();
            DB.confirmHandoverItemRelease(itemId, {
              cssd_release_confirmed_by_id: s.id, cssd_release_confirmed_by_name: s.name, cssd_release_confirmed_at: TrueTime.nowISO()
            }).then((result) => {
              UI.writeResultToast(result, 'Release confirmed');
              this._loadItemsReadyForRelease();
            }).catch(err => UI.toast('Could not confirm: ' + err.message, true));
          } else { onWrong(); }
        }
  ...
  async _renderRelease() {
    this._shell(`${this._sessionBar()}
      <div id="dp-wide-dashboard">
        ...
        <div class="table-wrap"><table>
          <thead><tr><th>Ref #</th><th>Submitted</th><th>Status</th></tr></thead>
          <tbody id="dp-wide-tbody"><tr><td colspan="3" class="empty-state">Loading…</td></tr></tbody>
        </table></div>
        <div id="dp-wide-pagination" style="margin-top:12px;"></div>
      </div>

      <div id="dp-narrow-sections">
      <div class="section-title" style="margin-top:0;">Items Ready for Release <span class="hint" style="font-weight:400;">both sides confirm with their own PIN — nothing is released until both are entered</span></div>
      <div id="dp-items-release-list">Loading…</div>

      <div class="section-title">Recently Released Items</div>
      <div id="dp-items-recent-release-list">Loading…</div>

      <div class="section-title">Pending — being processed by CSSD</div>
    ...
    this._loadItemsReadyForRelease();
    this._loadRecentlyReleasedItems();
  ...
  async _loadWideRelease(page = 'default') {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="3" class="empty-state">Couldn't load: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },
  ...
  _renderWideRelease() {
    ...
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="3" class="empty-state">No handovers match.</td></tr>`;
    } else {
      tbody.innerHTML = rows.map(r => {
        const isAlarming = HandoverView.isLateRelease(r);
        return `
        <tr class="${isAlarming ? 'row-alarm' : ''}">
          ...
          <td>
            ...
            <button class="btn btn-sm" style="background:none;border:none;color:var(--brand);" data-wide-quickedit="${r.id}">Details ▸</button>
            <button class="btn btn-sm" data-wide-print="${r.id}">Print</button>
          </td>
        </tr>
        <tr id="dp-wide-qe-${r.id}" style="display:none;"><td colspan="3" style="padding:0;background:var(--surface-sunken);"></td></tr>
      `;
      }).join('');
  ...
  _toggleWideQuickEdit(btn, r) {
    ...
    if (r.status === 'Released') {
      boxes += `<div class="card card-pad" style="margin-bottom:10px;"><div style="margin-bottom:8px;">${this._renderItemsList(r.load_contents)}</div><button class="btn btn-sm btn-primary" id="qe-verify">Verify Received</button></div>`;
    }
    if (r.has_pending_balance) {
      const short = (r.load_contents || []).filter(it => (it.received_qty ?? it.qty) < it.qty && !it.final_status);
      boxes += `<div class="card card-pad" style="margin-bottom:10px;border-left:3px solid var(--red);"><strong>Pending Balance</strong><div style="margin-top:8px;">${short.map(it => `
        <div style="margin-bottom:8px;display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
    ...
    td.innerHTML = `<div style="padding:14px 18px;">${boxes || this._renderItemsList(r.load_contents)}</div>`;
    const verifyBtn = document.getElementById('qe-verify');
    if (verifyBtn) verifyBtn.addEventListener('click', () => this._openVerifyModal(r));
