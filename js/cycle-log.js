...
const CycleLogView = {
    ...
    await UI.withLoading(btn, async () => {
      try {
        if (this._editingDraftId) {
          ...
          UI.writeResultToast(result, 'Committed — assigned a cycle number, ready to start');
        }
  ...
  _editDraft(row) {
    ...
    document.getElementById('cyc-machine').disabled = true; // machine's locked once committed — start a fresh draft to change it
  ...
  // Block committing a new Draft for a machine that already has one in
  ...
  _syncOpenWarning(machineId) {
    ...
    const open = (this._openByMachine && this._openByMachine[machineId]) || (this._draftRows || []).find(d => d.machine_id === machineId);
    if (open && !this._editingDraftId) {
