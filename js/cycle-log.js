...
const CycleLogView = {
  ...
  // Shared with the global alarm checker (scheduled-alarms.js) so a
  // cycle's exposure-due state is computed identically whether or not
  // anyone actually has this page open.
  computeExposureDue(cycle) {
    if (!cycle.time_start || !cycle.exposure_time_minutes) return { computable: false };
    const minutes = parseInt(cycle.exposure_time_minutes, 10);
    if (!minutes) return { computable: false };
    const dueAt = new Date(new Date(cycle.time_start).getTime() + minutes * 60000);
    return { computable: true, dueAt, isOverdue: TrueTime.now().getTime() >= dueAt.getTime() };
  },
  ...
  _tickCountdowns() {
    ...
    openRows.forEach(cycle => {
      ...
      const due = this.computeExposureDue(cycle);
      if (!due.computable) return;
      const remainingMs = due.dueAt.getTime() - TrueTime.now().getTime();
      ...
      if (!due.isOverdue) {
        ...
        const snoozed = Alarm.isSnoozed(cycle.alarm_acknowledged_at) || Alarm.isLocallyMuted(alarmKey);
        if (snoozed) {
          const resumesAt = new Date(new Date(cycle.alarm_acknowledged_at).getTime() + Alarm.snoozeMinutes * 60000);
          if (countdownEl) countdownEl.innerHTML = `⚠ EXPOSURE TIME COMPLETE <span class="hint" style="font-weight:400;">— muted by ${UI.escapeHtml(cycle.alarm_acknowledged_by)} at ${UI.fmtDateTime(cycle.alarm_acknowledged_at)}, resumes ${UI.fmtDateTime(resumesAt.toISOString())} if still unresolved</span>`;
          Alarm.stop(alarmKey);
        } else {
  ...
  async _startNewCycle() {
    ...
    const who = await IdentityConfirm.confirm('Start New Cycle');
    if (!who) return;
    await UI.withLoading(btn, async () => {
      try {
        const result = await DB.addCycle({ status: 'Draft', staff_id: who.id, staff_name: who.name });
  ...
  _openStopModal(row) {
    ...
    modal.querySelector('#cyc-stop-confirm').addEventListener('click', async () => {
      ...
      // Close this modal before opening IdentityConfirm's own — see
      // recurring bug pattern #17.
      UI.closeModal();
      const who = await IdentityConfirm.confirm('Stop Cycle');
      if (!who) return;
      try {
  ...
  async _startCycle(id) {
    ...
    const who = await IdentityConfirm.confirm('Start Cycle');
    if (!who) return;
    try {
      const result = await DB.updateCycle(id, {
        status: 'In Progress',
        time_start: TrueTime.nowISO(),
        operator_start: who.name,
        operator_start_id: who.id,
  ...
  _renderTimeline(container, cycle) {
    ...
    container.querySelector('[data-tl-flush]')?.addEventListener('click', () => this._advanceTimeline(cycle, { time_knob_to_flush: TrueTime.nowISO() }, 'flush', 'Confirm Flush'));
    container.querySelector('[data-tl-drying]')?.addEventListener('click', () => this._advanceTimeline(cycle, { time_knob_to_drying: TrueTime.nowISO() }, 'drying', 'Confirm Drying'));
    container.querySelector('[data-tl-hatch]')?.addEventListener('click', () => this._advanceTimeline(cycle, { time_knob_off_hatch_open: TrueTime.nowISO(), cooldown_start: TrueTime.nowISO() }, 'hatch', 'Confirm Open Hatch'));
    container.querySelector('[data-tl-cooldown]')?.addEventListener('click', () => this._advanceTimeline(cycle, { cooldown_end: TrueTime.nowISO() }, 'cooldown', 'Confirm Cool Down Complete'));
    container.querySelector('[data-tl-complete]')?.addEventListener('click', () => this._completeCycle(cycle));
  },

  async _advanceTimeline(cycle, fields, stepKey, actionLabel) {
    const who = await IdentityConfirm.confirm(actionLabel);
    if (!who) return;
    // Merge into whatever's already there rather than overwrite —
    // each step calls this independently, potentially at very
    // different times, and earlier steps' confirmations must survive.
    const confirmedBy = { ...(cycle.timeline_confirmed_by || {}), [stepKey]: { id: who.id, name: who.name } };
    fields.timeline_confirmed_by = confirmedBy;
    try {
  ...
  async _completeCycle(cycle) {
    ...
    const who = await IdentityConfirm.confirm('Complete Cycle');
    if (!who) return;
    fields.operator_end = who.name;
    fields.operator_end_id = who.id;
      ...
      document.getElementById(`cyc-bi-initiate-${cycle.id}`).addEventListener('click', async () => {
        ...
        try {
          const result = await DB.addQaTest({
            ...
            date_of_test: UI.todayStr(), time_of_test: UI.nowTimeStr(), operator: who.name,
