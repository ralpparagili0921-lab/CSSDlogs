...
const CycleLogView = {
  ...
  _renderPhase3Params(container, cycle) {
    ...
    document.getElementById(`cyc-p3-save-${cycle.id}`).addEventListener('click', async () => {
      ...
      fields.parameters_saved_at = new Date().toISOString();
      try {
  ...
  _wireCaptureField(modal, id) {
    modal.querySelector(`#cc-capture-${id}`).addEventListener('click', () => {
      const now = new Date();
      modal.querySelector(`#cc-${id}`).value = now.toISOString().slice(0, 16);
  ...
  async _renderTable(rows) {
    ...
    tbody.innerHTML = rows.map(r => {
      ...
      <tr id="cyc-details-row-${r.id}" style="display:none;"><td colspan="10" style="padding:14px 16px;background:var(--surface-sunken);">${this._renderCycleDetails(r)}</td></tr>
