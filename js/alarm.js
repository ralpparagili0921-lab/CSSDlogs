...
const Alarm = {
  ...
  _unlocked: false,
  _repeatTimers: {},
  _localMutedUntil: {}, // key -> timestamp ms — fallback for when the persisted DB timestamp fails to save (e.g. Supabase's schema-cache bug); NOT a replacement for it, just a safety net so a failed save can't immediately un-mute the alarm on the very next tick
  ...
  // and immediately resume alarming. Checked alongside (not instead
  // of) muteLocally()/isLocallyMuted() below.
  isSnoozed(acknowledgedAt) {
    if (acknowledgedAt) {
      const snoozedUntil = new Date(acknowledgedAt).getTime() + this.snoozeMinutes * 60000;
      if (TrueTime.now().getTime() < snoozedUntil) return true;
    }
    return false;
  },

  // The local fallback itself — set unconditionally the moment
  // someone mutes, regardless of whether the DB save that follows
  // succeeds. Every alarm-firing check should treat isSnoozed(...) ||
  // isLocallyMuted(key) as "currently muted," not isSnoozed() alone.
  muteLocally(key) {
    this._localMutedUntil[key] = TrueTime.now().getTime() + this.snoozeMinutes * 60000;
  },
  isLocallyMuted(key) {
    const until = this._localMutedUntil[key];
    if (!until) return false;
    if (TrueTime.now().getTime() >= until) { delete this._localMutedUntil[key]; return false; }
    return true;
  },
  ...
  // Tries Aria/Jenny first (the requested preference), then a broader
  // list of commonly-known female voice names across Windows, macOS,
  // and Chrome — since the Web Speech API has no actual "gender"
  // field to query directly, name-matching against known voices is
  // the only mechanism available. This can't be a true 100% guarantee
  // on every possible device (a system with zero recognizable female
  // voices installed has nothing for this to find), but covers the
  // large majority of real Windows/Mac/Chrome/Edge setups.
  _getVoice() {
    if (this._preferredVoice) return this._preferredVoice;
    if (!window.speechSynthesis) return null;
    const voices = window.speechSynthesis.getVoices();

    // A manually-chosen voice (via Admin -> Voice Settings) wins if
    // it's still actually available on this device — stored per-device
    // in localStorage, not the shared database, since voice
    // availability genuinely differs machine to machine.
    const manualChoice = localStorage.getItem('cssd_alarm_voice_override');
    if (manualChoice) {
      const chosen = voices.find(v => v.name === manualChoice);
      if (chosen) { this._preferredVoice = chosen; return chosen; }
      // Chosen voice no longer exists on this device (e.g. switched
      // devices, or the OS voice pack changed) — fall through to auto.
    }

    const names = ['aria', 'jenny', 'zira', 'samantha', 'victoria', 'karen', 'moira', 'susan', 'female'];
    let match = null;
    for (const name of names) {
      match = voices.find(v => v.name.toLowerCase().includes(name));
      if (match) break;
    }
    if (!match) match = voices.find(v => v.name.toLowerCase().includes('google') && v.name.toLowerCase().includes('us english')); // Chrome's default US voice is female
  ...
  // title/message describe what happened; onAcknowledge runs AFTER the
  // mute has already taken effect — it should persist whatever audit
  // record the caller needs (e.g. who/when, for cross-device sync),
  // but its failure never blocks or undoes the actual muting. The
  // whole point of muting is to stop the noise right now; a backend
  // hiccup (e.g. a database schema-cache bug entirely outside the
  // app's control) must never be able to prevent that.
  showBox(key, title, message, onAcknowledge) {
    ...
    `;
    box.querySelector('.alarm-box-ack').addEventListener('click', async () => {
      // Mute first, unconditionally — nothing below this line can
      // undo it. muteLocally() is the part that actually makes this
      // stick across future ticks even if the DB save below fails.
      this.muteLocally(key);
      this.stop(key);
      this.removeBox(key);
      try {
        await onAcknowledge();
      } catch (err) {
        // The mute already succeeded — this only means the
        // acknowledgment RECORD (who/when) didn't save. Worth
        // flagging, but not urgent enough to reopen the box or
        // resume the alarm over.
        UI.toast(`Muted — but couldn't save the acknowledgment record: ${err.message}`, true);
      }
    });
