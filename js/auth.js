...
const Auth = {
  ...
  async renderLogin() {
    ...
    card.innerHTML = `
      ...
      <button id="btn-handover-submit" style="width:100%;margin:18px 0;padding:20px 18px;border:none;border-radius:var(--radius);background:var(--brand);color:#fff;font-weight:800;font-size:16px;cursor:pointer;box-shadow:var(--shadow);">📦 Submit Instrument/Supplies for Sterilization<div style="font-weight:500;font-size:12.5px;opacity:.85;margin-top:4px;">For ER, OPD, OR, and Ward staff — no login needed</div></button>
    ...
    document.getElementById('btn-handover-submit').addEventListener('click', () => HandoverSubmitPortal.show());
    ...
    if (this._pinBuffer.length === 4) {
      if (stage === 1) {
        ...
        if (this._pinBuffer === this._newPin1) {
          ...
          else this._saveRecoveredPin(this._pinBuffer);
        } else {
