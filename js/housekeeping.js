...
const HousekeepingView = {
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="card card-pad">
        <form id="hk-form">
          ...
          <div class="form-grid">
            ...
            ${LateEntryReason.fieldHtml('hk-late-reason')}
          </div>
      ...
      <div class="section-title" style="display:flex;align-items:center;justify-content:space-between;">
        <span>Recent checklists <span class="count" id="hk-count">—</span></span>
        <button class="btn btn-sm" id="hk-view-history">View all history →</button>
      </div>
      <div class="table-wrap"><table>
        <thead><tr><th>Date</th><th>Type</th><th>Terminal?</th><th>Inspected by</th><th>Logged by</th><th></th></tr></thead>
        <tbody id="hk-tbody"><tr><td colspan="6" class="empty-state">Loading…</td></tr></tbody>
      </table></div>

      ${MissedEntriesView.sectionHtml('hk', 'Missed Entries')}
    `;
    ...
    LateEntryReason.wire('hk-late-reason', '15:30');
    document.getElementById('hk-view-history').addEventListener('click', () => App.navigate('hk-history'));
    MissedEntriesView.wire('hk-missed-range', (v) => this._loadMissedEntries(v));
    this._loadTable();
  },
  ...
  async _submit(e) {
    ...
    const lateErr = LateEntryReason.validateOnSubmit('hk-late-reason', '15:30');
    if (lateErr) { UI.toast(lateErr, true); return; }

    const who = await IdentityConfirm.confirm('Save Checklist');
    if (!who) return;
    ...
    const entry = {
      ...
      late_reason: LateEntryReason.reasonForSave('hk-late-reason', '15:30'),
      staff_id: who.id,
      staff_name: who.name
    };
  ...
  async _loadTable() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="6" class="empty-state">Couldn't load checklists: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },

  // Housekeeping's own schedule rule (matches missed-logs.js exactly):
  // due Monday-Friday, plus Sunday — but a Saturday cleaning covers
  // that week's Sunday, so Sunday isn't flagged if Saturday's already
  // logged. Saturday itself is never due. Weekends/exceptions/pre-
  // launch days route through WorkCalendar so this always agrees with
  // every other missed-log check in the app.
  async _loadMissedEntries(rangeValue) {
    const wrap = document.getElementById('hk-missed-wrap');
    wrap.innerHTML = `<div class="empty-state">Loading…</div>`;
    try {
      const to = UI.todayStr();
      let from;
      if (rangeValue === 'all') {
        from = (await WorkCalendar.launchDate()) || UI.daysAgoStr(365);
      } else {
        from = UI.daysAgoStr(parseInt(rangeValue, 10));
      }
      const logs = await DB.listHousekeepingLogs({ from, to, limit: 2000 });
      const assignments = await DB.listAssignments();
      const { assignedStaff, staffDetails } = MissedEntriesView.staffFor(assignments, 'housekeeping');
      const logsByDate = new Map();
      logs.forEach(r => { if (!logsByDate.has(r.log_date)) logsByDate.set(r.log_date, []); logsByDate.get(r.log_date).push(r); });
      const loggedDates = new Set(logsByDate.keys());

      const dueDates = [];
      let cursor = new Date(from + 'T00:00:00');
      const end = new Date(to + 'T00:00:00');
      while (cursor <= end) {
        const dateStr = UI.dateToStr(cursor);
        const weekday = cursor.getDay(); // 0=Sun, 6=Sat
        if (weekday === 6) { cursor.setDate(cursor.getDate() + 1); continue; } // Saturday never due
        let due;
        if (weekday === 0) {
          // Sunday — due unless an exception, or Saturday (the day
          // before) already covered it.
          const satStr = UI.dateToStr(new Date(cursor.getTime() - 86400000));
          due = !(await WorkCalendar.isException(dateStr)) && !loggedDates.has(satStr);
          const launch = await WorkCalendar.launchDate();
          if (launch && dateStr < launch) due = false;
        } else {
          due = await WorkCalendar.isWorkingDay(dateStr);
        }
        if (due) dueDates.push(dateStr);
        cursor.setDate(cursor.getDate() + 1);
      }

      // 3:30pm — matches the scheduled-alarm window's own end time,
      // the point already established elsewhere as "genuinely late."
      const entries = MissedEntriesView.classify(dueDates, logsByDate, '15:30');
      entries.forEach(e => {
        e.assignedStaff = assignedStaff;
        e.staffDetails = staffDetails;
        e.onPrint = e.tag === 'Late'
          ? () => this._printChecklist(e.row)
          : () => MissedEntriesView.printMissedNotice('Housekeeping', e.date, assignedStaff);
      });
      MissedEntriesView.render('hk-missed-wrap', 'hk-missed-count', entries);
    } catch (e) {
      MissedEntriesView.errorState('hk-missed-wrap', e.message);
    }
  },

  _renderTable(rows, tbodyId = 'hk-tbody', countId = 'hk-count') {
    const tbody = document.getElementById(tbodyId);
    document.getElementById(countId).textContent = `${rows.length} shown`;
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6" class="empty-state">No checklists logged in the last 30 days.</td></tr>`;
      return;
    }
    ...
    tbody.querySelectorAll('[data-print]').forEach(btn => {
      btn.addEventListener('click', () => this._printChecklist(rows.find(r => r.id === btn.dataset.print)));
    });
  },

  _printChecklist(r) {
    const groupSection = (heading, items) => ({
      heading,
      rows: (items || []).map(t => [t.item, t.status || '—'])
    });
    const sections = [
      { heading: 'Checklist Info', rows: [
        ['Date', UI.fmtDate(r.log_date)],
        ['Cleaning type', r.cleaning_type === 'Other' ? r.cleaning_type_other : r.cleaning_type],
        ['Terminal cleaning?', r.has_terminal_cleaning ? 'Yes' : 'No'],
        ['Terminal completed by', r.terminal_cleaning_completed_by],
        ['Inspected by', r.inspected_by],
        ['Logged by', r.staff_name]
      ]},
      groupSection('Tasks', r.tasks),
      groupSection('Sterilization Equipment', r.sterilization_equipment),
      groupSection('Post-Cleaning Tasks', r.post_cleaning_tasks)
    ];
    if (r.has_terminal_cleaning) sections.push(groupSection('Terminal Cleaning Tasks', r.terminal_cleaning_tasks));
    PrintReport.generate({ title: 'HOUSEKEEPING CHECKLIST REPORT', refNumber: UI.fmtDate(r.log_date), sections });
  }
};

const HousekeepingHistoryView = {
  async render() {
    const container = document.getElementById('view-hk-history');
    const { tableWrap, setCount } = HistoryView.renderShell({
      container, title: 'Housekeeping Checklists — Full History', backView: 'housekeeping',
    ...
    tableWrap.innerHTML = `
      <div class="table-wrap">
        <table>
          <thead><tr><th>Date</th><th>Type</th><th>Terminal?</th><th>Inspected by</th><th>Logged by</th><th></th></tr></thead>
          <tbody id="hk-hist-tbody"><tr><td colspan="6" class="empty-state">Loading…</td></tr></tbody>
        </table>
      </div>
    `;
  },
  ...
  async _load(filters, setCount) {
    const tbody = document.getElementById('hk-hist-tbody');
    try {
      const rows = await DB.listHousekeepingLogs({ from: filters.from, to: filters.to, limit: 1000 });
      const filtered = filters.search ? SearchBar.filter(rows, { text: filters.search }, 'log_date', ['cleaning_type', 'cleaning_type_other', 'inspected_by', 'staff_name']) : rows;
      setCount(filtered.length);
      HousekeepingView._renderTable.call(HousekeepingView, filtered, 'hk-hist-tbody', 'hv-count');
    } catch (e) {
      tbody.innerHTML = `<tr><td colspan="6" class="empty-state">Couldn't load history: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  }
};
