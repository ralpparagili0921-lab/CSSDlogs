...
const HousekeepingView = {
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="section-title" style="display:flex;align-items:center;justify-content:space-between;margin-top:22px;">
        <span>Missed Entries <span class="count" id="hk-missed-count">—</span></span>
        <select id="hk-missed-range" style="max-width:160px;">
      ...
      <div class="card card-pad" id="hk-missed-wrap"><div class="empty-state">Loading…</div></div>
    `;
    ...
    document.getElementById('hk-missed-range').addEventListener('change', (e) => this._loadMissedEntries(e.target.value));
    this._loadTable();
    this._loadMissedEntries('30');
  },
  ...
  async _loadMissedEntries(rangeValue) {
    ...
    const countEl = document.getElementById('hk-missed-count');
    ...
    try {
      ...
      const loggedDates = new Set(logs.map(r => r.log_date));
      ...
      while (cursor <= end) {
        ...
        if (due && !loggedDates.has(dateStr)) missed.push(dateStr);
        cursor.setDate(cursor.getDate() + 1);
      }

      countEl.textContent = `${missed.length} missed`;
      if (missed.length === 0) {
        wrap.innerHTML = `<div class="empty-state">No missed entries in this range.</div>`;
        return;
      }
      wrap.innerHTML = missed.sort().reverse().map(d => `
        <div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--line);">
          ...
          <span>${UI.fmtDate(d)}</span>
        </div>
      `).join('');
    } catch (e) {
      wrap.innerHTML = `<div class="empty-state">Couldn't load missed entries: ${UI.escapeHtml(e.message)}</div>`;
    }
  },
