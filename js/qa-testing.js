...
const QaTestingView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Ref #</th><th>Date</th><th>Machine</th><th>Test</th><th>Status</th><th>Key result</th><th>Operator</th></tr></thead>
        <tbody id="qa-tbody"><tr><td colspan="7" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
  ...
  async _loadTable() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="7" class="empty-state">Couldn't load tests: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },

  _renderTable(rows) {
    const tbody = document.getElementById('qa-tbody');
    document.getElementById('qa-count').textContent = `${rows.length} shown`;
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" class="empty-state">No tests match.</td></tr>`;
      return;
    }
