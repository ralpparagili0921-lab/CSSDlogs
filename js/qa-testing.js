    ...
    (this._machines || []).forEach(m => {
      ...
      if (!due.isOverdue) {
        ...
        if (snoozed) {
          ...
          if (countdownEl) countdownEl.innerHTML = `⚠ INCUBATION COMPLETE <span class="hint" style="font-weight:400;">— acknowledged ${UI.fmtDateTime(row.alarm_acknowledged_at)} by ${UI.escapeHtml(row.alarm_acknowledged_by)}, snoozed until ${UI.fmtDateTime(resumesAt.toISOString())}</span>`;
          Alarm.stop(alarmKey);
        } else {
