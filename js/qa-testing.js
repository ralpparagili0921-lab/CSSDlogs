...
const QaTestingView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      ${MissedEntriesView.sectionHtml('qa', 'Missed/Late Tests (BI + Dummy)')}
    `;
    document.getElementById('qa-view-history').addEventListener('click', () => App.navigate('qa-history'));
    MissedEntriesView.wire('qa-missed-range', (v) => this._loadMissedEntries(v));
  ...
  _renderBowieDickCard(m) {
    ...
    document.getElementById(`qa-bd-form-${m.machine_id}`).addEventListener('submit', async (e) => {
      ...
      const who = await IdentityConfirm.confirm('Save Bowie-Dick Result');
      if (!who) return;
      const entry = {
        machine_id: m.machine_id, test_type: 'Bowie-Dick', status: 'Completed',
        date_of_test: fd.get('date_of_test') || UI.todayStr(), time_of_test: UI.nowTimeStr(), operator: who.name,
  ...
  _renderDummyCard(m) {
      ...
      <div class="card card-pad" style="background:var(--surface-sunken);">
        ...
        <form id="qa-dummy-form-${m.machine_id}">
          <div class="form-grid">
            ...
            ${LateEntryReason.fieldHtml(`qa-dummy-late-${m.machine_id}`)}
          </div>
    ...
    LateEntryReason.wire(`qa-dummy-late-${m.machine_id}`, '08:00');
    document.getElementById(`qa-dummy-form-${m.machine_id}`).addEventListener('submit', async (e) => {
      e.preventDefault();
      const lateErr = LateEntryReason.validateOnSubmit(`qa-dummy-late-${m.machine_id}`, '08:00');
      if (lateErr) { UI.toast(lateErr, true); return; }
      const fd = new FormData(e.target);
      const failed = chipState.dummy_result === 'Fail' || chipState.dummy_ci_result === 'Fail';
      const who = await IdentityConfirm.confirm('Save Dummy/CI Result');
      if (!who) return;
      const entry = {
        ...
        late_reason: LateEntryReason.reasonForSave(`qa-dummy-late-${m.machine_id}`, '08:00'),
        staff_id: who.id, staff_name: who.name
      };
      const btn = e.target.querySelector('button[type="submit"]');
      await UI.withLoading(btn, async () => {
  ...
  async _tickBiCountdowns() {
    ...
    machines.forEach(m => {
      ...
      if (Alarm.isSnoozed(row.alarm_acknowledged_at) || Alarm.isLocallyMuted(`bi-incubation-${m.machine_id}`)) return;
    ...
    machines.forEach(m => {
      ...
      if (!due.isOverdue) {
        ...
        const snoozed = Alarm.isSnoozed(row.alarm_acknowledged_at) || Alarm.isLocallyMuted(alarmKey);
        if (snoozed) {
  ...
  _renderBiInitiate(host, m) {
      ...
      <div class="card card-pad" style="background:var(--surface-sunken);">
        ...
        <form id="qa-bi-init-form-${m.machine_id}">
          <div class="form-grid">
            ...
            ${LateEntryReason.fieldHtml(`qa-bi-late-${m.machine_id}`)}
          </div>
    ...
    LateEntryReason.wire(`qa-bi-late-${m.machine_id}`, '08:30');
    document.getElementById(`qa-bi-init-form-${m.machine_id}`).addEventListener('submit', async (e) => {
      e.preventDefault();
      const lateErr = LateEntryReason.validateOnSubmit(`qa-bi-late-${m.machine_id}`, '08:30');
      if (lateErr) { UI.toast(lateErr, true); return; }
      const fd = new FormData(e.target);
      const who = await IdentityConfirm.confirm('Initiate BI Test');
      if (!who) return;
      const entry = {
        machine_id: m.machine_id, test_type: 'BI', status: 'Incubating',
        date_of_test: fd.get('bi_incubation_date') || UI.todayStr(), time_of_test: UI.nowTimeStr(), operator: who.name,
        ...
        late_reason: LateEntryReason.reasonForSave(`qa-bi-late-${m.machine_id}`, '08:30'),
        staff_id: who.id, staff_name: who.name
      };
      const btn = e.target.querySelector('button[type="submit"]');
      await UI.withLoading(btn, async () => {
  ...
  _renderBiLogResult(host, m, row) {
    ...
    const buildForm = () => {
      ...
      document.getElementById(`qa-bi-result-form-${m.machine_id}`).addEventListener('submit', async (e) => {
        ...
        const who = await IdentityConfirm.confirm('Save BI Result');
        if (!who) return;
        const fields = {
          status: 'Completed',
          ...
          bi_read_by_id: who.id,
          bi_read_by_name: who.name
        };
        const btn = e.target.querySelector('button[type="submit"]');
        await UI.withLoading(btn, async () => {
          try {
  ...
  _renderBiPrelim(m, row) {
    ...
    host.querySelectorAll('[data-prelim]').forEach(btn => btn.addEventListener('click', async () => {
      const who = await IdentityConfirm.confirm('Record Preliminary Read');
      if (!who) return;
      try {
        const result = await DB.updateQaTest(row.id, {
          bi_prelim_result: btn.dataset.prelim, bi_prelim_read_at: TrueTime.nowISO(),
          bi_prelim_read_by_id: who.id, bi_prelim_read_by_name: who.name
        });
  ...
  // Combines both cadences into one box — BI (weekly, on each
  // machine's own scheduled day, "late" past 8:30am the same day) and
  // Dummy (daily, every working day, "late" past 8:00am) — since both
  // are QA Testing's responsibility and the user wants one unified
  // history rather than two separate boxes.
  async _loadMissedEntries(rangeValue) {
    try {
      ...
      const { assignedStaff, staffDetails } = MissedEntriesView.staffFor(assignments, 'qa');

      const allEntries = [];

      // BI — weekly, on each machine's own scheduled day.
      const biMachines = machines.filter(m => m.qa_schedule_day);
      const dayIndex = { Monday: 1, Tuesday: 2, Wednesday: 3, Thursday: 4, Friday: 5 };
      for (const m of biMachines) {
        const biRows = rows.filter(r => r.machine_id === m.machine_id && r.test_type === 'BI');
        const logsByDate = new Map();
        biRows.forEach(r => { if (!logsByDate.has(r.date_of_test)) logsByDate.set(r.date_of_test, []); logsByDate.get(r.date_of_test).push(r); });
        const dueDates = [];
        let cursor = new Date(from + 'T00:00:00');
        const end = new Date(to + 'T00:00:00');
        while (cursor <= end) {
          const dStr = UI.dateToStr(cursor);
          if (cursor.getDay() === dayIndex[m.qa_schedule_day] && await WorkCalendar.isWorkingDay(dStr)) {
            dueDates.push(dStr);
          }
          cursor.setDate(cursor.getDate() + 1);
        }
        const entries = MissedEntriesView.classify(dueDates, logsByDate, '08:30', 'Late', 'Missed');
        entries.forEach(e => {
          e.detail = `${e.detail ? e.detail + ' — ' : ''}${m.machine_id} BI`;
          e.assignedStaff = assignedStaff;
          e.staffDetails = staffDetails;
          e.onPrint = e.tag === 'Late' ? () => this._printTest(e.row) : () => MissedEntriesView.printMissedNotice(`QA BI — ${m.machine_id}`, e.date, assignedStaff);
          allEntries.push(e);
        });
      }

      // Dummy/CI — daily, every working day, for any machine that offers it.
      const dummyMachines = machines.filter(m => (m.applicable_tests || []).includes('Dummy'));
      for (const m of dummyMachines) {
        const dummyRows = rows.filter(r => r.machine_id === m.machine_id && r.test_type === 'Dummy');
        const logsByDate = new Map();
        dummyRows.forEach(r => { if (!logsByDate.has(r.date_of_test)) logsByDate.set(r.date_of_test, []); logsByDate.get(r.date_of_test).push(r); });
        const dueDates = [];
        let cursor = new Date(from + 'T00:00:00');
        const end = new Date(to + 'T00:00:00');
        while (cursor <= end) {
        ...
        const entries = MissedEntriesView.classify(dueDates, logsByDate, '08:00', 'Late', 'Missed');
        entries.forEach(e => {
          e.detail = `${e.detail ? e.detail + ' — ' : ''}${m.machine_id} Dummy`;
          e.assignedStaff = assignedStaff;
          e.staffDetails = staffDetails;
          e.onPrint = e.tag === 'Late' ? () => this._printTest(e.row) : () => MissedEntriesView.printMissedNotice(`QA Dummy — ${m.machine_id}`, e.date, assignedStaff);
          allEntries.push(e);
        });
      }

      MissedEntriesView.render('qa-missed-wrap', 'qa-missed-count', allEntries);
    } catch (e) {
      MissedEntriesView.errorState('qa-missed-wrap', e.message);
    }
  },
