  ...
  _tickBiCountdowns() {
    ...
    (this._machines || []).forEach(m => {
      ...
      if (!due.isOverdue) {
        ...
        if (row.alarm_acknowledged_at) {
          if (countdownEl) countdownEl.innerHTML = `⚠ INCUBATION COMPLETE <span class="hint" style="font-weight:400;">— acknowledged ${UI.fmtDateTime(row.alarm_acknowledged_at)} by ${UI.escapeHtml(row.alarm_acknowledged_by)}</span>`;
          Alarm.stop(alarmKey);
        } else {
          if (countdownEl) countdownEl.innerHTML = `⚠ INCUBATION COMPLETE`;
          Alarm.start(alarmKey, `B I incubation complete for ${m.machine_id}`, 'QA Alert');
