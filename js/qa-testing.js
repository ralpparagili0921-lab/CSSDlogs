  ...
  async _loadMissedEntries(rangeValue) {
    try {
      ...
      for (const m of biMachines) {
        ...
        entries.forEach(e => { e.detail = `${e.detail ? e.detail + ' — ' : ''}${m.machine_id} BI`; allEntries.push(e); });
      }
      ...
      for (const m of dummyMachines) {
        ...
        entries.forEach(e => { e.detail = `${e.detail ? e.detail + ' — ' : ''}${m.machine_id} Dummy`; allEntries.push(e); });
      }
