...
const QaTestingView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="section-title">Recent tests <span class="count" id="qa-count">—</span></div>
      <div class="table-wrap"><table>
        <thead><tr><th>Date</th><th>Machine</th><th>Test</th><th>Status</th><th>Key result</th><th>Operator</th></tr></thead>
        <tbody id="qa-tbody"><tr><td colspan="6" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
  ...
  async _loadCards() {
    ...
    try {
      const [allMachines, recent, incubating] = await Promise.all([
        DB.listAllMachines(),
        DB.listQaTests({ from: UI.daysAgoStr(60), limit: 500 }),
  ...
  _renderBowieDickCard(m) {
      ...
      <div class="card card-pad" style="background:var(--surface-sunken);">
        ...
        <form id="qa-bd-form-${m.machine_id}">
          <div class="form-grid">
            ...
            ${this._failActionFieldHtml('qa-bd', true)}
    ...
    const failState = this._wireFailActionChips('qa-bd');
    host.querySelectorAll('[data-chip-row] .radio-chip').forEach(chip => chip.addEventListener('click', () => {
      ...
      document.getElementById('qa-bd-action-wrap').classList.toggle('hidden', chip.dataset.val !== 'Fail');
    }));
    document.getElementById(`qa-bd-form-${m.machine_id}`).addEventListener('submit', async (e) => {
      ...
      const entry = {
        ...
        bd_fail_action_other: failState.selected.has('Other') ? (document.getElementById('qa-bd-action-other').value || null) : null,
      ...
      await UI.withLoading(btn, async () => {
        try {
          ...
          const result = await DB.addQaTest(entry); UI.writeResultToast(result, 'Bowie-Dick result saved'); await this._loadCards(); this._loadTable();
  ...
  _renderDummyCard(m) {
      ...
      <div class="card card-pad" style="background:var(--surface-sunken);">
        ...
        <form id="qa-dummy-form-${m.machine_id}">
          <div class="form-grid">
            ...
            ${this._failActionFieldHtml('qa-dummy', true)}
    ...
    const failState = this._wireFailActionChips('qa-dummy');
    const syncActionVisibility = () => {
      const failed = chipState.dummy_result === 'Fail' || chipState.dummy_ci_result === 'Fail';
      document.getElementById('qa-dummy-action-wrap').classList.toggle('hidden', !failed);
    };
    ...
    document.getElementById(`qa-dummy-form-${m.machine_id}`).addEventListener('submit', async (e) => {
      ...
      const entry = {
        ...
        dummy_fail_action_other: failState.selected.has('Other') ? (document.getElementById('qa-dummy-action-other').value || null) : null,
      ...
      await UI.withLoading(btn, async () => {
        try {
          ...
          const result = await DB.addQaTest(entry); UI.writeResultToast(result, 'Dummy/CI result saved'); await this._loadCards(); this._loadTable();
  ...
  _renderBiLogResult(host, m, row) {
    ...
    host.innerHTML = `
      <div class="card card-pad pending-highlight">
        <div class="section-title" style="margin:0 0 4px;">BI test — log result</div>
        <div style="font-size:12.5px;margin-bottom:14px;">${dueLine}</div>
    ...
    const buildForm = () => {
      formWrap.innerHTML = `
        <form id="qa-bi-result-form-${m.machine_id}" style="margin-top:12px;">
          <div class="form-grid">
            ...
            ${this._failActionFieldHtml('qa-bi', true)}
      ...
      `;
      const chipState = {};
      const failState = this._wireFailActionChips('qa-bi');
      document.getElementById(`qa-bi-final-${m.machine_id}`).addEventListener('change', (e) => {
        document.getElementById('qa-bi-action-wrap').classList.toggle('hidden', e.target.value !== 'FINAL FAIL');
      });
      ...
      document.getElementById(`qa-bi-result-form-${m.machine_id}`).addEventListener('submit', async (e) => {
        ...
        const fields = {
          ...
          bi_fail_action_other: failState.selected.has('Other') ? (document.getElementById('qa-bi-action-other').value || null) : null,
        ...
        await UI.withLoading(btn, async () => {
          try {
            const result = await DB.updateQaTest(row.id, fields); UI.writeResultToast(result, 'BI result saved'); await this._loadCards(); this._loadTable();
  ...
  _renderBiPrelim(m, row) {
    ...
    host.querySelectorAll('[data-prelim]').forEach(btn => btn.addEventListener('click', async () => {
      try {
        const result = await DB.updateQaTest(row.id, { bi_prelim_result: btn.dataset.prelim, bi_prelim_read_at: new Date().toISOString() });
        UI.writeResultToast(result, 'Preliminary read recorded');
        row.bi_prelim_result = btn.dataset.prelim;
        row.bi_prelim_read_at = new Date().toISOString();
  ...
  async _loadTable() {
    ...
    try {
      const rows = await DB.listQaTests({ from: UI.daysAgoStr(60), limit: 100 });
      document.getElementById('qa-count').textContent = `${rows.length} in last 60 days`;
      if (rows.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="empty-state">No QA tests logged in the last 60 days.</td></tr>`;
        return;
      }
      ...
      tbody.innerHTML = `<tr><td colspan="6" class="empty-state">Couldn't load tests: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  }
};
