...
// Shades the chart's plot area to show a normal range visually, not
// just via a thin reference line — pale red above/below the
// configured min/max, left untouched (white) in between. A native
// Chart.js plugin (registered once, configured per-chart via
// options.plugins.zoneShading) rather than the chartjs-plugin-
// annotation library, since this app is offline-first and every
// external dependency needs to be reliably cached for that — a
// plugin built from the already-loaded core avoids that risk
// entirely. Only meaningful for a single metric against a single
// location's range; see _loadChart's own comment on why it's skipped
// otherwise.
if (typeof Chart !== 'undefined') {
  Chart.register({
    id: 'zoneShading',
    beforeDraw(chart) {
      const opts = chart.options.plugins && chart.options.plugins.zoneShading;
      if (!opts || opts.min == null || opts.max == null) return;
      const yScale = chart.scales[opts.yAxisID];
      if (!yScale) return;
      const { ctx, chartArea } = chart;
      const minPx = yScale.getPixelForValue(opts.min);
      const maxPx = yScale.getPixelForValue(opts.max);
      ctx.save();
      ctx.fillStyle = 'rgba(196,67,46,0.10)';
      if (maxPx > chartArea.top) ctx.fillRect(chartArea.left, chartArea.top, chartArea.right - chartArea.left, maxPx - chartArea.top);
      if (minPx < chartArea.bottom) ctx.fillRect(chartArea.left, minPx, chartArea.right - chartArea.left, chartArea.bottom - minPx);
      ctx.restore();
    }
  });
}
...
const TempHumidityView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="card card-pad">
        <form id="th-form">
          ...
          <div class="form-grid" style="margin-top:14px;">
            ...
            ${LateEntryReason.fieldHtml('th-late-reason')}
          </div>
      ...
      <div class="section-title" style="display:flex;align-items:center;justify-content:space-between;">
        <span>Recent readings <span class="count" id="th-count">—</span></span>
        <button class="btn btn-sm" id="th-view-history">View all history →</button>
      </div>
      ...
      ${MissedEntriesView.sectionHtml('th-am', 'Missed/Late — AM Reading')}
      ${MissedEntriesView.sectionHtml('th-pm', 'Missed/Late — PM Reading')}
    ...
    document.getElementById('th-chart-range').addEventListener('change', () => this._loadChart());
    document.getElementById('th-chart-period').addEventListener('change', () => this._loadChart());
    this._loadChart();
    ...
    // Deadline depends on which reading is actually being logged right
    // now, not a single fixed time like the other logbooks — same
    // before-noon-is-AM convention as the deadline used in the Missed
    // Entries section of this file and the trend chart's AM/PM split.
    LateEntryReason.wire('th-late-reason', TrueTime.now().getHours() < 12 ? '08:00' : '16:00');
    ...
    document.getElementById('th-view-history').addEventListener('click', () => App.navigate('th-history'));
    MissedEntriesView.wire('th-am-missed-range', (v) => this._loadMissedEntries('am', v));
    MissedEntriesView.wire('th-pm-missed-range', (v) => this._loadMissedEntries('pm', v));
    this._loadTable();
  },
  ...
  async _submit(e) {
    e.preventDefault();
    const thDeadline = TrueTime.now().getHours() < 12 ? '08:00' : '16:00';
    const lateErr = LateEntryReason.validateOnSubmit('th-late-reason', thDeadline);
    if (lateErr) { UI.toast(lateErr, true); return; }
    ...
    const entry = {
      ...
      late_reason: LateEntryReason.reasonForSave('th-late-reason', thDeadline),
  ...
  async _loadChart() {
    ...
    const rangeDays = parseInt(document.getElementById('th-chart-range').value, 10) || 30;
    const periodFilter = document.getElementById('th-chart-period').value; // 'both' | 'AM' | 'PM'
    ...
    // A shaded normal-range background only makes unambiguous sense
    // against ONE range on ONE axis — a single metric, a single
    // location. Multiple locations can have different configured
    // ranges, and temp/humidity share no common scale, so either of
    // those falls back to a thicker reference line instead of a fill.
    const isSimpleView = !comparingBoth && metric !== 'both';
    try {
      const rows = await DB.listTempHumidityLogs({ from: UI.daysAgoStr(rangeDays), limit: 3000 });
      const groups = selected === 'both'
        ? this._locations.map(l => ({ name: l.name, readings: rows.filter(r => r.location_id === l.id), loc: l }))
        : [{ name: (this._locations.find(l => l.id === selected) || {}).name || '', readings: rows.filter(r => r.location_id === selected), loc: this._locations.find(l => l.id === selected) }];
      groups.forEach(g => g.readings.sort((a, b) => (a.log_date + (a.log_time || '')).localeCompare(b.log_date + (b.log_time || ''))));

      // Keyed by date+period, not just date — AM and PM are two
      // genuinely separate readings each day, and a plain date key
      // would silently let the later one (PM) overwrite the earlier
      // (AM) in the lookup below, discarding half the data. Same
      // am/pm-from-log_time convention already used by the Missed
      // Entries section of this file.
      const periodOf = t => (t || '') < '12:00:00' ? 'AM' : 'PM';
      const keyOf = r => `${r.log_date}|${periodOf(r.log_time)}`;
      if (periodFilter !== 'both') groups.forEach(g => { g.readings = g.readings.filter(r => periodOf(r.log_time) === periodFilter); });
      const allSlots = Array.from(new Set(groups.flatMap(g => g.readings.map(keyOf)))).sort();
      const slotLabels = allSlots.map(k => { const [d, p] = k.split('|'); return periodFilter === 'both' ? `${UI.fmtDate(d)} ${p}` : UI.fmtDate(d); });

      const datasets = [];
      let zoneOpts = null;
      groups.forEach((g, i) => {
        const byKey = {}; g.readings.forEach(r => { byKey[keyOf(r)] = r; });
        const color = this._chartColors[i % this._chartColors.length];
        const suffix = groups.length > 1 ? ` (${g.name})` : '';
        const refLineWidth = isSimpleView ? 1 : 3;
        ...
        if (showTemp) {
          if (comparingBoth) {
            ...
            datasets.push({
              ...
              data: allSlots.map(k => byKey[k] ? byKey[k].temperature_c : null),
            ...
            datasets.push({
              ...
              pointBackgroundColor: allSlots.map(k => byKey[k] ? (byKey[k].temp_pass ? color : '#8B0000') : color),
              pointRadius: allSlots.map(k => byKey[k] && !byKey[k].temp_pass ? 5 : 2)
            });
          }
          // Reference lines — drawn from each location's own configured
          // min/max, not a fixed constant, since different locations
          // can genuinely have different acceptable ranges.
          if (g.loc && g.loc.temp_min != null && g.loc.temp_max != null) {
            datasets.push({
              type: 'line', label: `Temp min/max${suffix}`, yAxisID: 'yTemp',
              data: allSlots.map(() => g.loc.temp_min), borderColor: color + '80', borderWidth: refLineWidth,
              borderDash: [2, 3], pointRadius: 0
            });
            datasets.push({
              type: 'line', label: `_hide_temp_max${suffix}`, yAxisID: 'yTemp',
              data: allSlots.map(() => g.loc.temp_max), borderColor: color + '80', borderWidth: refLineWidth,
              borderDash: [2, 3], pointRadius: 0
            });
            if (isSimpleView && metric === 'temp') zoneOpts = { min: g.loc.temp_min, max: g.loc.temp_max, yAxisID: 'yTemp' };
          }
        }
        if (showHumidity) {
          datasets.push({
            type: 'line', label: `Humidity %${suffix}`, yAxisID: 'yHumidity',
            data: allSlots.map(k => byKey[k] ? byKey[k].humidity_pct : null),
            borderColor: color, backgroundColor: 'transparent', borderDash: [5, 3], tension: 0.25, spanGaps: true,
            pointBackgroundColor: allSlots.map(k => byKey[k] ? (byKey[k].humidity_pass ? color : '#8B0000') : color),
            pointRadius: allSlots.map(k => byKey[k] && !byKey[k].humidity_pass ? 5 : 2)
          });
          if (g.loc && g.loc.humidity_min != null && g.loc.humidity_max != null) {
            datasets.push({
              type: 'line', label: `Humidity min/max${suffix}`, yAxisID: 'yHumidity',
              data: allSlots.map(() => g.loc.humidity_min), borderColor: color + '80', borderWidth: refLineWidth,
              borderDash: [1, 4], pointRadius: 0
            });
            datasets.push({
              type: 'line', label: `_hide_humidity_max${suffix}`, yAxisID: 'yHumidity',
              data: allSlots.map(() => g.loc.humidity_max), borderColor: color + '80', borderWidth: refLineWidth,
              borderDash: [1, 4], pointRadius: 0
            });
            if (isSimpleView && metric === 'humidity') zoneOpts = { min: g.loc.humidity_min, max: g.loc.humidity_max, yAxisID: 'yHumidity' };
          }
        }
      });
      ...
      if (showTemp) scales.yTemp = { type: 'linear', position: 'left', title: { display: true, text: '°C' }, grid: { display: false } };
      if (showHumidity) scales.yHumidity = { type: 'linear', position: showTemp ? 'right' : 'left', title: { display: true, text: '% RH' }, grid: { display: false } };
      ...
      this._chart = new Chart(canvas.getContext('2d'), {
        ...
        options: {
          ...
          // The "max" half of each min/max reference pair is hidden
          // from the legend (its own label already explains the pair).
          plugins: {
            legend: { display: true, position: 'bottom', labels: { filter: item => !item.text.startsWith('_hide_') } },
            zoneShading: zoneOpts
          }
        }
      });
  ...
  // period: 'am' or 'pm' — each location's AM/PM reading is its own
  // independent daily requirement, weekdays only. Splits logs by
  // actual log_time before/after noon, matching how
  // scheduled-alarms.js already distinguishes the two windows.
  async _loadMissedEntries(period, rangeValue) {
    const wrapId = `th-${period}-missed-wrap`;
    try {
      ...
      const rows = await DB.listTempHumidityLogs({ from, to, limit: 5000 });
      const periodRows = rows.filter(r => period === 'am' ? (r.log_time || '') < '12:00:00' : (r.log_time || '') >= '12:00:00');
      const assignments = await DB.listAssignments();
      const { assignedStaff, staffDetails } = MissedEntriesView.staffFor(assignments, `temp-humidity-${period}`);
      ...
      while (cursor <= end) {
        const dStr = UI.dateToStr(cursor);
        if (await WorkCalendar.isWorkingDay(dStr)) dueDates.push(dStr);
        cursor.setDate(cursor.getDate() + 1);
      }

      const deadline = period === 'am' ? '08:00' : '16:00';
      for (const loc of locations) {
        const locRows = periodRows.filter(r => r.location_id === loc.id);
        const logsByDate = new Map();
        locRows.forEach(r => { if (!logsByDate.has(r.log_date)) logsByDate.set(r.log_date, []); logsByDate.get(r.log_date).push(r); });
        const entries = MissedEntriesView.classify(dueDates, logsByDate, deadline);
        entries.forEach(e => {
          e.detail = `${e.detail ? e.detail + ' — ' : ''}${loc.name}`;
          e.assignedStaff = assignedStaff;
          e.staffDetails = staffDetails;
          e.onPrint = e.tag === 'Late' ? () => this._printReading(e.row) : () => MissedEntriesView.printMissedNotice(`Temp/Humidity ${period.toUpperCase()} — ${loc.name}`, e.date, assignedStaff);
          allEntries.push(e);
        });
      }

      MissedEntriesView.render(wrapId, `th-${period}-missed-count`, allEntries);
    } catch (e) {
      MissedEntriesView.errorState(wrapId, e.message);
    }
  },

  _renderTable(rows, tbodyId = 'th-tbody', countId = 'th-count') {
...
const TempHumidityHistoryView = {
  async render() {
    const container = document.getElementById('view-th-history');
    const { tableWrap, setCount } = HistoryView.renderShell({
      container, title: 'Temperature & Humidity Monitoring — Full History', backView: 'temp-humidity',
    ...
    tableWrap.innerHTML = `
      <div class="table-wrap">
        <table>
          <thead><tr><th>Ref #</th><th>Date</th><th>Time</th><th>Location</th><th>Temp (°C)</th><th>Humidity (%)</th><th>Action taken</th><th>Logged by</th><th>Remarks</th><th></th></tr></thead>
          <tbody id="th-hist-tbody"><tr><td colspan="10" class="empty-state">Loading…</td></tr></tbody>
        </table>
      </div>
    `;
  },
  ...
  async _load(filters, setCount) {
    const tbody = document.getElementById('th-hist-tbody');
    try {
      const rows = await DB.listTempHumidityLogs({ from: filters.from, to: filters.to, limit: 1000 });
      const filtered = filters.search ? SearchBar.filter(rows, { text: filters.search }, 'log_date', ['location_name', 'staff_name', 'remarks']) : rows;
      setCount(filtered.length);
      TempHumidityView._renderTable.call(TempHumidityView, filtered, 'th-hist-tbody', 'hv-count');
    } catch (e) {
      tbody.innerHTML = `<tr><td colspan="10" class="empty-state">Couldn't load history: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  }
};
