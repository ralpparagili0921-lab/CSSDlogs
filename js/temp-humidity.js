...
const TempHumidityView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Date</th><th>Time</th><th>Location</th><th>Temp (°C)</th><th>Humidity (%)</th><th>Action taken</th><th>Logged by</th><th>Remarks</th></tr></thead>
        <tbody id="th-tbody"><tr><td colspan="8" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
    ...
    document.getElementById('th-report-now').addEventListener('click', () => {
      this._abnormalityReportedAt = new Date().toISOString();
  ...
  async _loadTable() {
    ...
    try {
      ...
      document.getElementById('th-count').textContent = `${rows.length} in last 30 days`;
      if (rows.length === 0) {
        tbody.innerHTML = `<tr><td colspan="8" class="empty-state">No readings logged in the last 30 days.</td></tr>`;
        return;
      }
      ...
      tbody.innerHTML = `<tr><td colspan="8" class="empty-state">Couldn't load readings: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  }
};
