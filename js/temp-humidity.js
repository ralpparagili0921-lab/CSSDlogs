...
const TempHumidityView = {
  ...
  async _loadChart() {
    ...
    // AM+PM, both locations, a full year — generous ceiling so a wide
    // range never silently truncates.
    try {
      ...
      const slotLabels = allSlots.map(k => { const [d, p] = k.split('|'); return `${UI.fmtDate(d)} ${p}`; });
        ...
        if (showTemp) {
          ...
          if (g.loc && g.loc.temp_min != null && g.loc.temp_max != null) {
            datasets.push({
              ...
              data: allSlots.map(() => g.loc.temp_min), borderColor: color + '80', borderWidth: 1,
              borderDash: [2, 3], pointRadius: 0
            });
            datasets.push({
              type: 'line', label: `_hide_temp_max${suffix}`, yAxisID: 'yTemp',
              data: allSlots.map(() => g.loc.temp_max), borderColor: color + '80', borderWidth: 1,
              borderDash: [2, 3], pointRadius: 0
            });
          }
        }
        if (showHumidity) {
          ...
          if (g.loc && g.loc.humidity_min != null && g.loc.humidity_max != null) {
            datasets.push({
              ...
              data: allSlots.map(() => g.loc.humidity_min), borderColor: color + '80', borderWidth: 1,
              borderDash: [1, 4], pointRadius: 0
            });
            datasets.push({
              type: 'line', label: `_hide_humidity_max${suffix}`, yAxisID: 'yHumidity',
              data: allSlots.map(() => g.loc.humidity_max), borderColor: color + '80', borderWidth: 1,
              borderDash: [1, 4], pointRadius: 0
            });
          }
        }
