...
const TempHumidityView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Ref #</th><th>Date</th><th>Time</th><th>Location</th><th>Temp (°C)</th><th>Humidity (%)</th><th>Action taken</th><th>Logged by</th><th>Remarks</th></tr></thead>
        <tbody id="th-tbody"><tr><td colspan="9" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
  ...
  async _loadTable() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="9" class="empty-state">Couldn't load readings: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },
  ...
  _renderTable(rows) {
    ...
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="9" class="empty-state">No readings match.</td></tr>`;
      return;
    }
