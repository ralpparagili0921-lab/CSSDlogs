...
const DB = {
  ...
  async confirmHandoverIntake(id, items, hasDiscrepancy) {
    return OfflineQueue.submit('update', 'instrument_handovers', {
      load_contents: items,
      intake_verified_at: TrueTime.nowISO(),
      intake_verified_by_id: Auth.currentStaff.id,
      intake_verified_by_name: Auth.currentStaff.name,
      has_intake_discrepancy: hasDiscrepancy
    }, id);
  },
  async notifyIntakeDiscrepancy(id) {
    return OfflineQueue.submit('update', 'instrument_handovers', {
      intake_discrepancy_notified_at: TrueTime.nowISO(),
      intake_discrepancy_notified_by_name: Auth.currentStaff.name
    }, id);
  },
