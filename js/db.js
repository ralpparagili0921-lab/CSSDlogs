...
// falls back to a manual v4 UUID for any other testing context (e.g.
// opening index.html directly via file://) where it's unavailable.
...
const DB = {
  ...
  async reportError(entry) {
    // Direct call, not through the offline queue — if the app itself is
    // in enough trouble to be reporting an error, queuing it behind
    // everything else risks losing it. If this fails too (e.g. genuinely
    // offline), that's handled by the caller showing a plain toast instead.
    const { error } = await supabaseClient.from('error_reports').insert(entry);
    if (error) throw error;
  },
  async listErrorReports() {
    const { data, error } = await supabaseClient.from('error_reports').select('*').order('created_at', { ascending: false });
    if (error) throw error;
    return data;
  },
  async updateErrorReport(id, fields) {
    return OfflineQueue.submit('update', 'error_reports', fields, id);
  },
  ...
  async addCycle(entry) {
    ...
    // Generate the id client-side rather than relying on the DB to hand
    // one back — OfflineQueue.submit() never returns inserted row data
    // (and can't, when queued offline there's no server round-trip yet),
    // so this is the only reliable way the caller gets an id immediately.
    const id = generateUUID();
    const result = await OfflineQueue.submit('insert', 'sterilization_cycles', { ...entry, id, cycle_number });
    return { ...result, id, cycle_number };
  },
  async updateCycle(id, fields) {
    return OfflineQueue.submit('update', 'sterilization_cycles', fields, id);
  },
  async deleteCycleDraft(id) {
    return OfflineQueue.submit('delete', 'sterilization_cycles', null, id);
  },
  // Runs opportunistically whenever someone visits Cycle Log — finds
  // any reservation that was never actually committed (machine_id
  // still null) and is older than 24h, and deletes it so the cycle
  // number frees up. A direct call rather than going through the
  // offline queue, since this inherently needs connectivity just to
  // know what's orphaned in the first place — if offline, it simply
  // gets another chance the next time someone with a connection opens
  // this page. Failing silently is correct here, not a bug to surface.
  async cleanupOrphanedCycleDrafts(excludeId) {
    const cutoff = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    try {
      let q = supabaseClient.from('sterilization_cycles').delete()
        .eq('status', 'Draft').is('machine_id', null).lt('created_at', cutoff);
      if (excludeId) q = q.neq('id', excludeId);
      const { error } = await q;
      if (error) throw error;
    } catch (e) { /* best-effort — next visitor with connectivity gets another chance */ }
  },
  ...
  // Per-item tracking (handover_items) — only ever called once BOTH
  // the department and CSSD PIN confirmations are met for that item;
  // there is no earlier, partial insert. Also keeps the batch
  // record's own load_contents JSONB in sync (read-modify-write,
  // since OfflineQueue.submit overwrites fields rather than appending)
  // so the existing wide-dashboard/status-badge/intake-verification
  // code — all built around load_contents — keeps working unchanged.
  async addHandoverItem(handoverId, item) {
    return OfflineQueue.submit('insert', 'handover_items', { ...item, handover_id: handoverId });
  },
  async appendHandoverLoadContentsItem(handoverId, item) {
    const { data, error } = await supabaseClient.from('instrument_handovers').select('load_contents').eq('id', handoverId).single();
    if (error) throw error;
    const items = data.load_contents || [];
    items.push({ id: generateUUID(), name: item.item_name, qty: item.qty });
    return OfflineQueue.submit('update', 'instrument_handovers', { load_contents: items }, handoverId);
  },
  // Release pathway — fully submitted items (both Submission PINs
  // already met) not yet marked ready by CSSD.
  // Distinguishes a batch created via the new per-item flow (has rows
  // here) from a genuinely old-style batch (never will) — the old
  // batch-level dashboard uses this to exclude new-style batches from
  // sections gated on intake_verified_at, a field the new flow never
  // sets, since those batches' real progress lives in handover_items
  // instead and would otherwise look permanently stuck.
  async listHandoverIdsWithItems() {
    const { data, error } = await supabaseClient.from('handover_items').select('handover_id');
    if (error) throw error;
    return new Set(data.map(r => r.handover_id));
  },
  async listHandoverItemsAwaitingReady() {
    const { data, error } = await supabaseClient.from('handover_items').select('*')
      .not('cssd_submit_confirmed_at', 'is', null).is('ready_for_release_at', null).order('created_at', { ascending: true });
    if (error) throw error;
    return data;
  },
  // Broader than the above — every item still "in progress" at CSSD
  // regardless of whether it's been marked ready yet, for the
  // per-item turnaround alarm (which needs to track from submission
  // all the way to full release, not just up to the ready-marking
  // step).
  async listHandoverItemsInProgress() {
    const { data, error } = await supabaseClient.from('handover_items').select('*')
      .not('cssd_submit_confirmed_at', 'is', null).is('cssd_release_confirmed_at', null).order('created_at', { ascending: true });
    if (error) throw error;
    return data;
  },
  async markHandoverItemReady(itemId, fields) {
    return OfflineQueue.submit('update', 'handover_items', fields, itemId);
  },
  // Items CSSD has marked ready, scoped to one department — release
  // confirmation still pending on one or both sides. Two simple
  // queries (get this department's batch ids, then items belonging to
  // them) rather than a cross-table filter, since that join syntax
  // isn't otherwise proven to work anywhere in this codebase and this
  // app has no way to test it before it's actually deployed live.
  async listHandoverItemsReadyForRelease(department) {
    const { data: batches, error: batchErr } = await supabaseClient.from('instrument_handovers').select('id').eq('department', department);
    if (batchErr) throw batchErr;
    const batchIds = batches.map(b => b.id);
    if (batchIds.length === 0) return [];
    const { data, error } = await supabaseClient.from('handover_items').select('*')
      .not('ready_for_release_at', 'is', null).is('cssd_release_confirmed_at', null)
      .in('handover_id', batchIds).order('ready_for_release_at', { ascending: true });
    if (error) throw error;
    return data;
  },
  async confirmHandoverItemRelease(itemId, fields) {
    return OfflineQueue.submit('update', 'handover_items', fields, itemId);
  },
  // Items fully released (both Release PINs confirmed) for one
  // department — nothing previously surfaced this after the fact,
  // an item just left the "ready for release" list with no further
  // visibility. Same two-query pattern as listHandoverItemsReadyForRelease
  // above, for the same reason — no proven cross-table join precedent
  // in this codebase.
  async listRecentlyReleasedItemsForDepartment(department, limit = 30) {
    const { data: batches, error: batchErr } = await supabaseClient.from('instrument_handovers').select('id').eq('department', department);
    if (batchErr) throw batchErr;
    const batchIds = batches.map(b => b.id);
    if (batchIds.length === 0) return [];
    const { data, error } = await supabaseClient.from('handover_items').select('*')
      .not('cssd_release_confirmed_at', 'is', null)
      .in('handover_id', batchIds).order('cssd_release_confirmed_at', { ascending: false }).limit(limit);
    if (error) throw error;
    return data;
  },
  // CSSD-side equivalent of the above — no department scope, since
  // CSSD staff serve every department, not just one. Still needs a
  // second query for each item's batch (department, serial_number),
  // since handover_items itself doesn't store department directly —
  // same two-query approach as everywhere else this same need has
  // come up, since there's still no proven cross-table join
  // precedent in this codebase.
  async listRecentlyReleasedItems(limit = 30) {
    const { data: items, error } = await supabaseClient.from('handover_items').select('*')
      .not('cssd_release_confirmed_at', 'is', null).order('cssd_release_confirmed_at', { ascending: false }).limit(limit);
    if (error) throw error;
    if (items.length === 0) return [];
    const batchIds = [...new Set(items.map(it => it.handover_id))];
    const { data: batches, error: batchErr } = await supabaseClient.from('instrument_handovers').select('id, department, department_other, serial_number').in('id', batchIds);
    if (batchErr) throw batchErr;
    const batchById = new Map(batches.map(b => [b.id, b]));
    return items.map(it => ({ ...it, _batch: batchById.get(it.handover_id) || null }));
  },
  ...
  async listIntakeDiscrepanciesForDepartment(department) {
    const { data, error } = await supabaseClient.from('instrument_handovers').select('*').eq('department', department).eq('has_intake_discrepancy', true).not('intake_discrepancy_notified_at', 'is', null).order('intake_discrepancy_notified_at', { ascending: false });
    if (error) throw error;
    return data;
  },
  ...
  async listReceivedForDepartment(department, limit = 30) {
    const { data, error } = await supabaseClient.from('instrument_handovers').select('*').eq('status', 'Received').eq('department', department).order('received_verified_at', { ascending: false }).limit(limit);
    if (error) throw error;
    return data;
  },
  ...
  // CSSD physically confirming what was actually received against what
  // the department claimed to submit — separate from verifyHandoverReceived
  // above, which is the DEPARTMENT'S later verification of what CSSD
  // released back to them. items is the full, updated load_contents
  // array (each item now carrying intake_qty/intake_remarks).
  async confirmHandoverIntake(id, items, hasDiscrepancy, who) {
    return OfflineQueue.submit('update', 'instrument_handovers', {
      load_contents: items,
      intake_verified_at: TrueTime.nowISO(),
      intake_verified_by_id: who.id,
      intake_verified_by_name: who.name,
      has_intake_discrepancy: hasDiscrepancy
    }, id);
  },
  async notifyIntakeDiscrepancy(id, who) {
    return OfflineQueue.submit('update', 'instrument_handovers', {
      intake_discrepancy_notified_at: TrueTime.nowISO(),
      intake_discrepancy_notified_by_id: who.id,
      intake_discrepancy_notified_by_name: who.name
    }, id);
  },
  ...
  async listHandoversForDepartment(department, { from, to, limit } = {}) {
    let q = supabaseClient.from('instrument_handovers').select('*').eq('department', department).order('received_at', { ascending: false });
    if (from) q = q.gte('received_at', from);
    if (to) q = q.lte('received_at', to + 'T23:59:59');
    if (limit) q = q.limit(limit);
    const { data, error } = await q;
