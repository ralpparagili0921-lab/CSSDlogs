...
const DB = {
  ...
  async listActiveStaff() {
    return cacheReadThrough('cssd_cache_active_staff', async () => {
      const { data, error } = await supabaseClient.from('staff').select('*').eq('active', true).order('name');
      if (error) throw error;
      return data;
    });
  },
  async listAllStaff() {
    const { data, error } = await supabaseClient.from('staff').select('*').order('active', { ascending: false }).order('name');
    if (error) throw error;
    return data;
  },
  ...
  async addStaff(fields) {
    const { error } = await supabaseClient.from('staff').insert(fields);
    if (error) throw error;
  },
  ...
  async listAssignments() {
    const { data, error } = await supabaseClient.from('logbook_assignments').select('*, staff:staff_id(name)');
    if (error) throw error;
    return data;
  },
  async setAssignment(logbook, staff_id) {
    const { error } = await supabaseClient.from('logbook_assignments').update({ staff_id }).eq('logbook', logbook);
    if (error) throw error;
  },
  ...
  async addRoLog(entry) {
    return OfflineQueue.submit('insert', 'ro_water_quality', entry);
  },
  ...
  async addDowntimeLog(entry) {
    return OfflineQueue.submit('insert', 'equipment_downtime', entry);
  },
  ...
  async addCycle(entry) {
    return OfflineQueue.submit('insert', 'sterilization_cycles', entry);
  },
  ...
  async addQaTest(entry) {
    return OfflineQueue.submit('insert', 'sterilizer_qa_tests', entry);
  },
  ...
  async addInstrumentMaintenance(entry) {
    return OfflineQueue.submit('insert', 'instrument_maintenance', entry);
  },
  ...
  async addHandoverIntake(entry) {
    return OfflineQueue.submit('insert', 'instrument_handovers', entry);
  },
  ...
  async addTempHumidityLog(entry) {
    return OfflineQueue.submit('insert', 'temp_humidity_logs', entry);
  },
