...
const RoView = {
  ...
  async render() {
      ...
      <div class="card card-pad">
        <form id="ro-form">
          ...
          <div class="form-grid" style="margin-top:14px;">
            ...
            ${LateEntryReason.fieldHtml('ro-late-reason')}
          </div>
    ...
    if (activateBtn) {
      activateBtn.addEventListener('click', async () => {
        const who = await IdentityConfirm.confirm('Activate RO Monitoring');
        if (!who) return;
        try {
          await DB.updateAppMeta({
            ro_monitoring_activated: true, ro_monitoring_activated_at: TrueTime.nowISO(),
            ro_monitoring_activated_by_id: who.id, ro_monitoring_activated_by_name: who.name
          });
    ...
    LateEntryReason.wire('ro-late-reason', '14:30');
  ...
  _addTesterModal() {
    ...
    modal.querySelector('#rt-save').addEventListener('click', async () => {
      ...
      const model = modal.querySelector('#rt-model').value || null;
      const make = modal.querySelector('#rt-make').value || null;
      // Close this modal before opening IdentityConfirm's own — see
      // recurring bug pattern #17.
      UI.closeModal();
      const who = await IdentityConfirm.confirm('Add Tester');
      if (!who) return;
      try {
        await DB.addRoTester({ name, model, make, added_by_id: who.id, added_by_name: who.name });
  ...
  async _submit(e) {
    e.preventDefault();
    const lateErr = LateEntryReason.validateOnSubmit('ro-late-reason', '14:30');
    ...
    const entry = {
      ...
      late_reason: LateEntryReason.reasonForSave('ro-late-reason', '14:30'),
      staff_id: who.id,
      staff_name: who.name
    };
  ...
  async _loadMissedEntries(rangeValue) {
    try {
      ...
      const { assignedStaff, staffDetails } = MissedEntriesView.staffFor(assignments, 'ro');
      ...
      while (cursor <= end) {
        dueDates.push(UI.dateToStr(cursor)); // daily, every day, no weekend exclusion for RO
      ...
      entries.forEach(e => {
        ...
        e.onPrint = e.tag === 'Late' ? () => this._printEntry(e.row) : () => MissedEntriesView.printMissedNotice('RO Water Quality', e.date, assignedStaff);
      });
