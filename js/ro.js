...
const RoView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr id="ro-thead-row"><th>Date</th><th>Time</th><th>Tester</th></tr></thead>
  ...
  _renderParamFields() {
    ...
    wrap.innerHTML = this._parameters.map(p => `
      <div class="field">
        ...
        <input type="number" step="0.01" data-param-id="${p.id}" placeholder="${p.reference_note ? UI.escapeHtml(p.reference_note) : (p.limit_min != null && p.limit_max != null ? p.limit_min + '–' + p.limit_max : p.limit_max != null ? '< ' + p.limit_max : '')}">
      </div>
    `).join('');
  },
  ...
  async _loadTable() {
    ...
    try {
      ...
      document.getElementById('ro-count').textContent = `${rows.length} in last 30 days`;
      ...
      document.getElementById('ro-thead-row').innerHTML =
        `<th>Date</th><th>Time</th><th>Tester</th>` +
        paramNames.map(n => `<th>${UI.escapeHtml(n)}</th>`).join('') +
        `<th>Logged by</th><th>Remarks</th>`;

      if (rows.length === 0) {
        tbody.innerHTML = `<tr><td class="empty-state">No entries yet in the last 30 days.</td></tr>`;
        return;
      }
