...
const AdminView = {
  ...
  async _loadAssignments() {
    ...
    try {
      ...
      wrap.innerHTML = Object.keys(LOGBOOK_LABELS).map(key => {
        const current = assignments.find(a => a.logbook === key);
        return `
          <div class="field">
            <label>${LOGBOOK_LABELS[key]}</label>
  ...
  async _saveAssignments() {
    ...
    try {
      for (const sel of selects) {
        await DB.setAssignment(sel.dataset.assign, sel.value || null);
      }
  ...
  async _loadMachines() {
    ...
    try {
      ...
      const typeLabel = { ro: 'RO System', flash_sterilizer: 'Flash Sterilizer', autoclave: 'Autoclave' };
  ...
  _machineModal(existing) {
    ...
    const syncQaVisibility = () => qaField.classList.toggle('hidden', typeSelect.value === 'ro');
