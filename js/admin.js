...
const AdminView = {
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="section-title">Voice Settings <span class="hint" style="font-weight:400;">this device only</span></div>
      <div class="card card-pad">
        <div class="hint" style="margin-bottom:10px;max-width:520px;">Scans this specific browser/device for every voice it has installed — availability genuinely differs machine to machine. By default the app auto-picks Aria, or the closest match it can find; you can override that here for this device specifically.</div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
          <select id="voice-select" style="min-width:260px;max-width:100%;"><option value="">Loading voices…</option></select>
          <button class="btn btn-sm" id="voice-save">Save for this device</button>
          <button class="btn btn-sm" id="voice-test">▶ Test</button>
        </div>
        <div class="hint" style="margin-top:8px;" id="voice-current-label"></div>
      </div>
    ...
    document.getElementById('voice-save').addEventListener('click', () => {
      const chosen = document.getElementById('voice-select').value;
      if (chosen) localStorage.setItem('cssd_alarm_voice_override', chosen);
      else localStorage.removeItem('cssd_alarm_voice_override');
      Alarm._preferredVoice = null; // clear the cache so the new choice actually takes effect right away, not just after a refresh
      this._updateVoiceCurrentLabel();
      UI.toast(chosen ? `This device will now use ${chosen}` : 'Reset to automatic voice selection');
    });
    document.getElementById('voice-test').addEventListener('click', () => {
      const chosen = document.getElementById('voice-select').value;
      const voices = window.speechSynthesis ? window.speechSynthesis.getVoices() : [];
      const testVoice = chosen ? voices.find(v => v.name === chosen) : Alarm._getVoice();
      Alarm._tone();
      const utterance = new SpeechSynthesisUtterance('This is a test of the alarm voice, Autoclave A C dash 0 1, exposure time is complete');
      if (testVoice) utterance.voice = testVoice;
      utterance.rate = 0.95; utterance.volume = 1;
  ...
  // Scanning voices is inherently async in some browsers — the first
  // call to getVoices() can return an empty list, with the real list
  // only arriving once the voiceschanged event fires. Handles both:
  // populates immediately with whatever's available, then re-populates
  // if the event fires with a fuller list afterward.
  _loadVoiceOptions() {
    const populate = () => {
      if (!window.speechSynthesis) return;
      const voices = window.speechSynthesis.getVoices();
      if (voices.length === 0) return; // wait for voiceschanged instead
      const sel = document.getElementById('voice-select');
      if (!sel) return; // navigated away before voices finished loading
      const saved = localStorage.getItem('cssd_alarm_voice_override') || '';
      sel.innerHTML = `<option value="">— Automatic (app picks the best match) —</option>` +
        voices.map(v => `<option value="${UI.escapeHtml(v.name)}" ${v.name === saved ? 'selected' : ''}>${UI.escapeHtml(v.name)} (${UI.escapeHtml(v.lang)})</option>`).join('');
      this._updateVoiceCurrentLabel();
    };
    populate();
    window.speechSynthesis.onvoiceschanged = () => { Alarm._preferredVoice = null; populate(); };
  },

  _updateVoiceCurrentLabel() {
    const el = document.getElementById('voice-current-label');
    if (!el) return;
    const voice = Alarm._getVoice();
    el.textContent = voice ? `Currently speaking as: ${voice.name}` : 'Currently speaking as: browser default (nothing matched on this device)';
  },
  ...
  async _openCleanupModal() {
    ...
    const cutoffStr = UI.dateToStr(cutoff);
