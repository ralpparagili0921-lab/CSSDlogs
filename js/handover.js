...
const HandoverView = {
    ...
    el.innerHTML = `
      ...
      <div id="ho-wide-dashboard">
        ...
        <div class="table-wrap"><table>
          <thead><tr><th>Ref #</th><th>Department</th><th>Submitted by</th><th>Received</th><th>Status</th></tr></thead>
          <tbody id="ho-wide-tbody"><tr><td colspan="5" class="empty-state">Loading…</td></tr></tbody>
        </table></div>
      ...
      <div class="section-title">Items — mark ready for release <span class="count" id="ho-items-ready-count">—</span> <span class="hint" style="font-weight:400;">once marked, its two release PINs become available on the department portal</span></div>
      <div id="ho-items-ready-list"></div>

      <div class="section-title">Recently Released Items <span class="hint" style="font-weight:400;">across every department</span></div>
      <div id="ho-items-recent-release-list"></div>
  ...
  computeStatusBadges(r) {
    ...
    if (r.status === 'Received') {
      ...
      // Set together with status: 'Received' in the same DB write
      // (see DepartmentPortal._openVerifyModal) — this can never
      // genuinely apply while status is still 'Released', despite
      // that being the more obvious place to check it.
      ...
      if (this._itemHandoverIds && this._itemHandoverIds.has(r.id)) {
        // A batch from the new per-item flow — intake_verified_at is
        // never set by that flow at all, so without this check every
        // one of these would show "Awaiting Intake" forever here even
        // once its real per-item progress has moved well past that.
        // This dashboard doesn't (yet) show per-item detail, so this
        // stays honestly generic rather than trying to summarize it.
        badges.push({ label: 'Per-Item Tracking', tone: 'neutral' });
      } else if (!r.intake_verified_at) {
  ...
  async _loadIntake() {
    ...
    try {
      const itemHandoverIds = await DB.listHandoverIdsWithItems();
      const rows = (await DB.listProcessingHandovers()).filter(r => !r.intake_verified_at && !itemHandoverIds.has(r.id));
      ...
      wrap.querySelectorAll('[data-intake-confirm]').forEach(btn => btn.addEventListener('click', async () => {
        ...
        const who = await IdentityConfirm.confirm('Confirm Received');
        if (!who) return;
        try {
          const result = await DB.confirmHandoverIntake(handoverId, items, hasDiscrepancy, who);
  ...
  async _loadIntakeDiscrepancies() {
    ...
    try {
      ...
      wrap.querySelectorAll('[data-intake-notify]').forEach(btn => btn.addEventListener('click', async () => {
        const who = await IdentityConfirm.confirm('Notify Department');
        if (!who) return;
        try {
          const result = await DB.notifyIntakeDiscrepancy(btn.dataset.intakeNotify, who);
  ...
  // Release pathway — items whose Submission PINs are both already
  // confirmed but that CSSD hasn't yet marked ready. Marking ready is
  // what makes the two Release PIN buttons appear on the department
  // portal — this is deliberately a separate, explicit step, not
  // automatic, since a submitted item may still be mid-sterilization.
  async _loadItemsAwaitingReady() {
    const wrap = document.getElementById('ho-items-ready-list');
    try {
      const rows = await DB.listHandoverItemsAwaitingReady();
      document.getElementById('ho-items-ready-count').textContent = rows.length;
      if (rows.length === 0) {
        wrap.innerHTML = `<div class="card card-pad empty-state" style="padding:18px;">Nothing waiting to be marked ready.</div>`;
        return;
      }
      wrap.innerHTML = rows.map(r => `
        <div class="card card-pad" style="margin-bottom:10px;display:flex;align-items:center;justify-content:space-between;gap:14px;flex-wrap:wrap;">
          <div>
            <strong>${UI.escapeHtml(r.item_name)}</strong> <span class="hint">× ${r.qty}</span>
            <div class="hint" style="margin-top:4px;">submitted ${UI.fmtDateTime(r.cssd_submit_confirmed_at)}</div>
          </div>
          <button class="btn btn-sm btn-primary" data-mark-ready="${r.id}">Mark Ready for Release</button>
        </div>
      `).join('');
      wrap.querySelectorAll('[data-mark-ready]').forEach(b => b.addEventListener('click', async () => {
        const who = await IdentityConfirm.confirm('Mark Ready for Release');
        if (!who) return;
        try {
          const result = await DB.markHandoverItemReady(b.dataset.markReady, {
            ready_for_release_by_id: who.id,
            ready_for_release_by_name: who.name,
            ready_for_release_at: TrueTime.nowISO()
          });
          UI.writeResultToast(result, 'Marked ready for release');
          this._loadItemsAwaitingReady();
        } catch (e) { UI.toast('Failed: ' + e.message, true); }
      }));
    } catch (e) {
  ...
  // CSSD-side equivalent of the department portal's own "Recently
  // Released Items" — spans every department, since CSSD staff serve
  // all of them, not just one. Without this, an item just leaves
  // CSSD's own view the moment it's marked ready, with nothing
  // showing it was ever actually completed.
  async _loadRecentlyReleasedItems() {
    const wrap = document.getElementById('ho-items-recent-release-list');
    if (!wrap) return;
    try {
      const rows = await DB.listRecentlyReleasedItems();
      if (rows.length === 0) {
      ...
      wrap.innerHTML = rows.map(r => `
        <div class="card card-pad" style="margin-bottom:10px;">
          ...
          <div class="hint" style="margin:4px 0;">${r._batch ? UI.escapeHtml(this._deptLabel(r._batch)) + ' · ' + UI.escapeHtml(r._batch.serial_number) : 'Unknown batch'}</div>
          ${r.has_discrepancy
  ...
  async _loadOpen() {
    ...
    try {
      ...
      // something into normal processing. This already, naturally
      // excludes batches from the new per-item flow too, without
      // needing an explicit check here — intake_verified_at is never
      // set by that flow at all, so this same condition already
      // covers it (unlike _loadIntake, which specifically needs the
      // opposite — items missing this field — and so needs the
      // explicit exclusion to tell the two apart).
      ...
      wrap.querySelectorAll('[data-release]').forEach(b => b.addEventListener('click', async () => {
        const who = await IdentityConfirm.confirm('Mark Released');
        if (!who) return;
        try {
          const result = await DB.releaseHandover(b.dataset.release, {
            status: 'Released',
            released_by_id: who.id,
            released_by_name: who.name,
  ...
  async _loadWideDashboard(page = 'default') {
    ...
    try {
      ...
      rows.forEach(r => { r._itemsSearchText = (r.load_contents || []).map(it => it.name).join(' '); });
      this._wideRows = rows;
      // Fetched once here, not inside computeStatusBadges() itself —
      // that function is synchronous and called once per row per
      // render, so it can't make its own async DB call. Same set used
      // by _loadIntake()/_loadOpen() to distinguish a batch made via
      // the new per-item flow (never sets intake_verified_at) from a
      // genuinely old-style one, so this dashboard doesn't show the
      // misleading "Awaiting Intake" badge for the new kind forever.
      this._itemHandoverIds = await DB.listHandoverIdsWithItems();
      this._renderWideDashboard();
    } catch (e) {
      tbody.innerHTML = `<tr><td colspan="5" class="empty-state">Couldn't load: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },
  ...
  _renderWideDashboard() {
    ...
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="5" class="empty-state">No handovers match.</td></tr>`;
    } else {
      tbody.innerHTML = rows.map((r, i) => {
        const isAlarming = this.isLateRelease(r);
        return `
        ...
        <tr id="ho-wide-qe-${r.id}" style="display:none;"><td colspan="5" style="padding:0;background:var(--surface-sunken);width:1px;overflow-x:auto;"></td></tr>
      `;
      }).join('');
  ...
  // Shows every event box that actually applies to this specific
  // transaction — a single reference can genuinely have several
  // (an intake shortfall that's already been notified, AND a
  // separate pending-balance situation found later, say) — not just
  // whichever one happens first. Each box only appears if its own
  // condition is actually true for this record.
  _toggleWideQuickEdit(btn, r) {
    ...
    // Box: still awaiting CSSD's own intake count — excludes batches
    // from the new per-item flow, which never sets intake_verified_at
    // at all (see computeStatusBadges' same exclusion above) — those
    // would otherwise show this box, complete with a working "Confirm
    // Received" button that writes to the OLD system's fields,
    // alongside their real, separate per-item progress.
    if (r.status === 'Processing' && !r.intake_verified_at && !(this._itemHandoverIds && this._itemHandoverIds.has(r.id))) {
      boxes += `
        <div class="card card-pad ${isAlarming ? 'card-alarm' : ''}" style="margin-bottom:10px;">
          <strong>Awaiting Intake Verification</strong>
          ...
          <div style="margin-top:8px;"><button class="btn btn-sm btn-primary" id="qe-confirm-intake" style="display:block;width:100%;text-align:center;">Confirm Received</button></div>
        </div>`;
    }

    // Box: an intake shortfall, either still needing notification or
    // already handled — the latter shown as history, not an action.
    // Only the "not yet notified" case can be the alarm's actual
    // cause — history is settled, nothing to glow about.
    if (r.has_intake_discrepancy) {
      const shortItems = (r.load_contents || []).filter(it => (it.intake_qty || 0) < it.qty);
      if (!r.intake_discrepancy_notified_at) {
        boxes += `
          <div class="card card-pad ${isAlarming ? 'card-alarm' : ''}" style="margin-bottom:10px;${isAlarming ? '' : 'border-left:3px solid var(--red);'}">
            <strong>Intake Discrepancy — Not Yet Notified</strong>
            <ul style="margin:8px 0 10px 18px;">${shortItems.map(it => `<li>${UI.escapeHtml(it.name)} — submitted ${it.qty}, received ${it.intake_qty || 0}</li>`).join('')}</ul>
            <div style="margin-top:8px;"><button class="btn btn-sm btn-primary" id="qe-notify">Notify Department</button></div>
          </div>`;
      } else {
        boxes += `
          <div class="card card-pad" style="margin-bottom:10px;">
            <strong>Intake Discrepancy</strong> <span class="hint">— department notified ${UI.fmtDateTime(r.intake_discrepancy_notified_at)}</span>
            <ul style="margin:8px 0 0 18px;">${shortItems.map(it => `<li>${UI.escapeHtml(it.name)} — submitted ${it.qty}, received ${it.intake_qty || 0}</li>`).join('')}</ul>
          </div>`;
      }
    }

    // Box: intake verified, no unnotified discrepancy — ready for CSSD to release
    if (r.status === 'Processing' && r.intake_verified_at && (!r.has_intake_discrepancy || r.intake_discrepancy_notified_at)) {
      boxes += `<div class="card card-pad ${isAlarming ? 'card-alarm' : ''}" style="margin-bottom:10px;"><button class="btn btn-sm btn-primary" id="qe-mark-released">Mark Released</button></div>`;
    }

    // Box: pending balance — same 3-stage workflow as the narrow-screen
    // section (awaiting CSSD review / awaiting department reverify /
    // unresolved awaiting a superuser's final call), scoped to just
    // this record's own items instead of every handover at once.
    if (r.has_pending_balance) boxes += this._renderBalanceBoxForHandover(r);

    td.innerHTML = `<div style="padding:14px 18px;">${boxes || this._renderHandoverDetails(r)}</div>`;

    const reload = () => this._loadWideDashboard(this._widePage);
    const confirmBtn = document.getElementById('qe-confirm-intake');
    if (confirmBtn) confirmBtn.addEventListener('click', async () => {
      ...
      if (!who) return;
      try {
        const result = await DB.confirmHandoverIntake(r.id, items, hasDiscrepancy, who);
        UI.writeResultToast(result, 'Confirmed');
        reload();
      } catch (e) { UI.toast('Failed: ' + e.message, true); }
    });
    const notifyBtn = document.getElementById('qe-notify');
    if (notifyBtn) notifyBtn.addEventListener('click', async () => {
      const who = await IdentityConfirm.confirm('Notify Department');
      if (!who) return;
      try {
        const result = await DB.notifyIntakeDiscrepancy(r.id, who);
        UI.writeResultToast(result, 'Department notified');
        reload();
      } catch (e) { UI.toast('Failed: ' + e.message, true); }
    });
    const releaseBtn = document.getElementById('qe-mark-released');
    if (releaseBtn) releaseBtn.addEventListener('click', async () => {
      const who = await IdentityConfirm.confirm('Mark Released');
      if (!who) return;
      try {
        const result = await DB.releaseHandover(r.id, {
          status: 'Released', released_by_id: who.id,
          released_by_name: who.name, released_at: TrueTime.nowISO()
        });
        UI.writeResultToast(result, 'Marked Released');
        reload();
      } catch (e) { UI.toast('Failed: ' + e.message, true); }
    });
    td.querySelectorAll('[data-resolve-item]').forEach(b => b.addEventListener('click', () => this._resolveItem(...b.dataset.resolveItem.split('|'), 'resolved').then(reload)));
    td.querySelectorAll('[data-unresolve-item]').forEach(b => b.addEventListener('click', () => this._openUnresolvedModal(...b.dataset.unresolveItem.split('|'))));
    td.querySelectorAll('[data-final]').forEach(b => {
      const [handoverId, itemId, status] = b.dataset.final.split('|');
      b.addEventListener('click', () => this._openFinalModal(handoverId, itemId, status));
    });
  },

  // Per-item, since one handover can have some items already resolved
  // and others still at a different stage — same 3-stage split as the
  // narrow-screen Pending Balance section, just for one record.
  _renderBalanceBoxForHandover(r) {
    const isSuperuser = Auth.currentStaff.role === 'superuser';
    let html = '';
    (r.load_contents || []).forEach(it => {
      if ((it.received_qty ?? it.qty) >= it.qty) return;
      if (it.final_status) return;
      if (it.awaiting_reverification) {
        html += `
          <div style="margin-bottom:8px;padding:10px;background:rgba(27,110,120,0.06);border-radius:var(--radius);">
            ${UI.escapeHtml(it.name)} — released remaining ${it.reverify_qty}, waiting on the department to confirm receipt.
          </div>`;
      } else if (!it.cssd_action) {
        html += `
          <div style="margin-bottom:8px;padding:10px;background:var(--surface-sunken);border-radius:var(--radius);display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap;">
            <div>${UI.escapeHtml(it.name)} — received ${it.received_qty} of ${it.qty}${it.verify_remarks ? ` · "${UI.escapeHtml(it.verify_remarks)}"` : ''}</div>
            <div style="display:flex;gap:8px;">
              <button class="btn btn-sm btn-primary" data-resolve-item="${r.id}|${it.id}">Resolve — release remaining</button>
              <button class="btn btn-sm" data-unresolve-item="${r.id}|${it.id}">Unresolved</button>
            </div>
          </div>`;
      } else if (it.cssd_action === 'unresolved') {
        html += `
          <div style="margin-bottom:8px;padding:10px;background:rgba(196,67,46,0.06);border-radius:var(--radius);">
            <div>${UI.escapeHtml(it.name)} — received ${it.received_qty} of ${it.qty}</div>
            <div class="hint">CSSD remarks: ${UI.escapeHtml(it.cssd_remarks) || '—'} · by ${UI.escapeHtml(it.cssd_action_by)}</div>
            ${isSuperuser ? `
              <div style="display:flex;gap:8px;margin-top:8px;">
                <button class="btn btn-sm btn-primary" data-final="${r.id}|${it.id}|final_resolved">Final Resolved</button>
                <button class="btn btn-sm" data-final="${r.id}|${it.id}|final_unresolved">Final Unresolved</button>
              </div>` : `<div class="hint" style="margin-top:6px;">Only a superuser can make this final.</div>`}
          </div>`;
      }
    });
    return html ? `<div class="card card-pad" style="margin-bottom:10px;border-left:3px solid var(--red);"><strong>Pending Balance</strong><div style="margin-top:8px;">${html}</div></div>` : '';
  },
  ...
  _renderLateTable(rows) {
    ...
    tbody.innerHTML = rows.map(r => `
      ...
      <tr id="ho-late-details-row-${r.id}" style="display:none;"><td colspan="6" style="padding:14px 16px;background:var(--surface-sunken);width:1px;overflow-x:auto;">${this._renderHandoverDetails(r)}</td></tr>
    `).join('');
  ...
  _renderTable(rows, tbodyId = 'ho-tbody', countId = 'ho-count') {
    ...
    tbody.innerHTML = rows.map(r => `
        ...
        <tr id="ho-details-row-${r.id}" style="display:none;"><td colspan="7" style="padding:14px 16px;background:var(--surface-sunken);width:1px;overflow-x:auto;">${this._renderHandoverDetails(r)}</td></tr>
      `).join('');
  ...
  _printHandover(r) {
    const rows = [
      ...
      ['Intake Verified', r.intake_verified_at ? `${UI.fmtDateTime(r.intake_verified_at)} by ${r.intake_verified_by_name || '—'}` : 'Not yet verified'],
      ['Intake Discrepancy', r.has_intake_discrepancy ? (r.intake_discrepancy_notified_at ? `Yes — department notified ${UI.fmtDateTime(r.intake_discrepancy_notified_at)}` : 'Yes — not yet notified') : 'None'],
    ...
    const itemRows = items.map(it => {
      ...
      if (it.intake_qty != null) line += `, intake ${it.intake_qty}${it.intake_remarks ? ` (${it.intake_remarks})` : ''}`;
    ...
    PrintReport.generate({
      ...
      sections: [{ heading: 'Handover Details', rows }, { heading: 'Items — Full Transaction History', rows: itemRows.length ? itemRows : [['Items', '(none listed)']] }]
    });
  }
};
