...
const HandoverView = {
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr><th>Ref #</th><th>Department</th><th>Submitted by</th><th>Status</th><th>Received</th><th></th></tr></thead>
  ...
  async _resolveItem(handoverId, itemId, action, remarks) {
    try {
      ...
      const updatedItems = (handover.load_contents || []).map(it => {
        ...
        const base = { ...it, cssd_action: action, cssd_remarks: remarks || null, cssd_action_at: new Date().toISOString(), cssd_action_by: Auth.currentStaff.name };
  ...
  _renderTable(rows) {
    ...
    tbody.innerHTML = rows.map(r => `
        ...
        <tr id="ho-details-row-${r.id}" style="display:none;"><td colspan="6" style="padding:14px 16px;background:var(--surface-sunken);">${this._renderHandoverDetails(r)}</td></tr>
      `).join('');
