    ...
    try {
      ...
      rows.forEach(r => {
        (r.load_contents || []).forEach(it => {
          ...
          if (!it.cssd_action) awaitingReview.push({ handover: r, item: it });
          else if (it.cssd_action === 'unresolved') unresolved.push({ handover: r, item: it });
        });
      });

      document.getElementById('ho-balance-count').textContent = awaitingReview.length;
      bWrap.innerHTML = awaitingReview.length === 0
        ? `<div class="card card-pad empty-state" style="padding:14px;">Nothing awaiting review.</div>`
        : awaitingReview.map(({ handover, item }) => `
  ...
  async _resolveItem(handoverId, itemId, action, remarks) {
    try {
      ...
      const updatedItems = (handover.load_contents || []).map(it => it.id === itemId
        ? { ...it, cssd_action: action, cssd_remarks: remarks || null, cssd_action_at: new Date().toISOString(), cssd_action_by: Auth.currentStaff.name }
        : it);
      ...
      UI.writeResultToast(result, action === 'resolved' ? 'Marked resolved' : 'Marked unresolved');
      this._loadBalance();
  ...
  _openFinalModal(handoverId, itemId, status) {
    ...
    modal.querySelector('#ho-final-submit').addEventListener('click', async () => {
      ...
      try {
        ...
        if (!handover) return;
        const updatedItems = (handover.load_contents || []).map(it => it.id === itemId
          ? { ...it, final_status: status, final_remarks: remarks, final_at: new Date().toISOString(), final_by: Auth.currentStaff.name }
          : it);
        const stillOpen = updatedItems.some(it => (it.received_qty ?? it.qty) < it.qty && !it.final_status);
        const result = await DB.updateHandoverItems(handoverId, updatedItems, stillOpen);
  ...
  _renderTable(rows) {
    ...
    tbody.innerHTML = rows.map(r => `
        <tr class="${r.status === 'Processing' ? 'pending-highlight' : ''}">
          ...
          <td>${r.load_contents && r.load_contents.length ? `<button class="btn btn-sm" data-items-toggle="${r.id}">Items ▸</button>` : '—'}</td>
        ...
        <tr id="ho-items-row-${r.id}" style="display:none;"><td colspan="8" style="padding:12px 16px;background:var(--surface-sunken);">${this._renderItemsList(r.load_contents)}</td></tr>
      `).join('');
