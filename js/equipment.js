...
const EquipmentView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr>
          <th>Machine</th><th>Type</th><th>Reason</th><th>Broken</th><th>Reported</th><th>Biomed response</th><th>Back up</th><th>Root cause</th><th>Remarks</th><th>Status</th><th></th>
        </tr></thead>
        <tbody id="eq-tbody"><tr><td colspan="11" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
  ...
  async _loadTable() {
    ...
    try {
      ...
      document.getElementById('eq-count').textContent = `${rows.length} in last 60 days`;
      if (rows.length === 0) {
        tbody.innerHTML = `<tr><td colspan="11" class="empty-state">No incidents logged in the last 60 days.</td></tr>`;
        return;
      }
      tbody.innerHTML = rows.map(r => `
        <tr>
          ...
          <td>${r.downtime_reason ? UI.escapeHtml(r.downtime_reason) : '—'}</td>
          <td>${UI.fmtDateTime(r.time_broken)}</td>
          <td>${UI.fmtDateTime(r.time_reported)}</td>
          <td>${UI.fmtDateTime(r.time_biomed_response)}</td>
          <td>${UI.fmtDateTime(r.time_up)}</td>
          <td>${r.root_cause_category ? UI.escapeHtml(r.root_cause_category) : '—'}</td>
          <td>${r.remarks ? UI.escapeHtml(r.remarks) : '—'}</td>
      ...
      tbody.innerHTML = `<tr><td colspan="11" class="empty-state">Couldn't load incidents: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  }
};
