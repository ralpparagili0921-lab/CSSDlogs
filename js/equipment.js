...
const EquipmentView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="section-title" style="display:flex;align-items:center;justify-content:space-between;">
        <span>Recent incidents <span class="count" id="eq-count">—</span></span>
        <button class="btn btn-sm" id="eq-view-history">View all history →</button>
      </div>
      ${SearchBar.render('eq-search')}
      <div class="table-wrap"><table>
        <thead><tr>
          <th>Ref #</th><th>Machine</th><th>Type</th><th>Broken</th><th>Status</th><th></th><th></th><th></th>
        </tr></thead>
        <tbody id="eq-tbody"><tr><td colspan="8" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
    ...
    FormDraft.attach(document.getElementById('eq-form'), 'eq-form');
    document.getElementById('eq-reason').dispatchEvent(new Event('change')); // re-sync the "Other" field's visibility if a restored draft had it selected
    document.getElementById('eq-view-history').addEventListener('click', () => App.navigate('equipment-history'));
    this._loadOpen();
    this._loadTable();
  ...
  async _submit(e) {
    ...
    const now = TrueTime.nowISO();
    const who = await IdentityConfirm.confirm('Log Incident');
    if (!who) return;
    const entry = {
    ...
    await UI.withLoading(btn, async () => {
      try {
        ...
        e.target.reset();
        FormDraft.clear('eq-form');
        document.getElementById('eq-reason-other').classList.add('hidden');
        this._loadOpen();
        this._loadTable();
      } catch (err) {
    ...
    try {
      ...
      wrap.querySelectorAll('[data-biomed]').forEach(btn => {
        btn.addEventListener('click', async () => {
          const who = await IdentityConfirm.confirm('Confirm Biomed/Facilities Responded');
          if (!who) return;
          try {
            const result = await DB.updateDowntimeLog(btn.dataset.biomed, {
              time_biomed_response: TrueTime.nowISO(),
              biomed_response_confirmed_by_id: who.id,
              biomed_response_confirmed_by_name: who.name
            });
  ...
  _openResolveModal(row) {
    // datetime-local inputs interpret their value as LOCAL time, but
    // toISOString() always produces UTC — using it directly here meant
    // the default "now" shown was 8 hours behind actual local time for
    // a Philippines-based user, easy to miss and submit as-is.
    ...
    modal.querySelector('#rs-save').addEventListener('click', async () => {
      // Read every field and close this modal BEFORE opening
      // IdentityConfirm's own — UI.showModal() always creates a new
      // element with id="active-modal", and closeModal() only ever
      // removes whichever one getElementById finds first, so with
      // two modals open at once, IdentityConfirm closing itself would
      // actually close THIS modal instead, leaving its own stuck open.
      const fields = {
      ...
      UI.closeModal();
      const who = await IdentityConfirm.confirm('Resolve Incident');
      if (!who) return;
      fields.resolved_by_id = who.id;
      fields.resolved_by_name = who.name;
      try {
  ...
  async _loadTable() {
    ...
    try {
      const rows = await DB.listDowntimeLogs({ from: UI.daysAgoStr(30), limit: 100 });
      ...
      tbody.innerHTML = `<tr><td colspan="8" class="empty-state">Couldn't load incidents: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },

  _renderTable(rows, tbodyId = 'eq-tbody', countId = 'eq-count') {
    const tbody = document.getElementById(tbodyId);
    document.getElementById(countId).textContent = `${rows.length} shown`;
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="8" class="empty-state">No incidents match.</td></tr>`;
      return;
    }
    tbody.innerHTML = rows.map(r => `
        ...
        <tr id="eq-details-row-${r.id}" style="display:none;"><td colspan="8" style="padding:14px 16px;background:var(--surface-sunken);">${this._renderIncidentDetails(r)}</td></tr>`).join('');
      tbody.querySelectorAll('[data-resolve2]').forEach(btn => {
        btn.addEventListener('click', () => this._openResolveModal(rows.find(r => r.id === btn.dataset.resolve2)));
      });
      ...
      tbody.querySelectorAll('[data-print]').forEach(btn => {
        btn.addEventListener('click', () => this._printIncident(rows.find(r => r.id === btn.dataset.print)));
      });
  },
    ...
    PrintReport.generate({
      ...
      sections: [{
        ...
        rows: [
          ...
          ['Type', r.machine_type === 'ro' ? 'RO System' : r.machine_type === 'flash_sterilizer' ? 'Flash Sterilizer' : r.machine_type === 'facility_equipment' ? 'Facility Equipment' : 'Autoclave'],
          ['Reason', r.downtime_reason],
          ['Broken', UI.fmtDateTime(r.time_broken)],
          ['Reported', UI.fmtDateTime(r.time_reported)],
          ['Biomed responded', UI.fmtDateTime(r.time_biomed_response)],
          ['Back in service', UI.fmtDateTime(r.time_up)],
          ['Root cause', r.root_cause_category],
          ['Remarks', r.remarks],
          ['Status', r.status]
        ]
      }]
...
const EquipmentHistoryView = {
  async render() {
    const container = document.getElementById('view-equipment-history');
    const { tableWrap, setCount } = HistoryView.renderShell({
      container, title: 'Equipment Downtime — Full History', backView: 'equipment',
      onFilterChange: (filters) => this._load(filters, setCount)
    });
    tableWrap.innerHTML = `
      <div class="table-wrap">
        <table>
          <thead><tr><th>Ref #</th><th>Machine</th><th>Type</th><th>Broken</th><th>Status</th><th></th><th></th><th></th></tr></thead>
          <tbody id="eq-hist-tbody"><tr><td colspan="8" class="empty-state">Loading…</td></tr></tbody>
        </table>
      </div>
    `;
  },
  ...
  async _load(filters, setCount) {
    const tbody = document.getElementById('eq-hist-tbody');
    try {
      const rows = await DB.listDowntimeLogs({ from: filters.from, to: filters.to, limit: 1000 });
      const filtered = filters.search ? SearchBar.filter(rows, { text: filters.search }, 'time_broken', ['machine_id', 'downtime_reason', 'remarks']) : rows;
      setCount(filtered.length);
      EquipmentView._renderTable.call(EquipmentView, filtered, 'eq-hist-tbody', 'hv-count');
    } catch (e) {
