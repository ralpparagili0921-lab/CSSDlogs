...
const EquipmentView = {
  ...
  async render() {
    ...
    el.innerHTML = `
      ...
      <div class="table-wrap"><table>
        <thead><tr>
          <th>Ref #</th><th>Machine</th><th>Type</th><th>Reason</th><th>Broken</th><th>Reported</th><th>Biomed response</th><th>Back up</th><th>Root cause</th><th>Remarks</th><th>Status</th><th></th>
        </tr></thead>
        <tbody id="eq-tbody"><tr><td colspan="12" class="empty-state">Loading…</td></tr></tbody>
      </table></div>
    `;
  ...
  async _loadTable() {
    ...
    try {
      ...
      tbody.innerHTML = `<tr><td colspan="12" class="empty-state">Couldn't load incidents: ${UI.escapeHtml(e.message)}</td></tr>`;
    }
  },
  ...
  _renderTable(rows) {
    ...
    if (rows.length === 0) {
      tbody.innerHTML = `<tr><td colspan="12" class="empty-state">No incidents match.</td></tr>`;
      return;
    }
