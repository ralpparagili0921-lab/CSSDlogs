      ...
      while (weekStart <= endDate) {
        ...
        for (const b of brushes) {
          ...
          if (weekRows.length > 0) {
            ...
            if (isLate) allEntries.push({ date: weekStartStr, tag: 'Late', detail: `${b.brush_id} — logged ${UI.fmtDate(earliest.slice(0, 10))} ${UI.fmtTime(earliest)}` });
          } else if (weekEndStr < today || (weekEndStr === today && nowHHMM >= '23:30')) {
            allEntries.push({ date: weekStartStr, tag: 'Missed', detail: `${b.brush_id} — week of ${UI.fmtDate(weekStartStr)}` });
          }
        }
