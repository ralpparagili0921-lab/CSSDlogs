...
const BrushView = {
    ...
    try {
      ...
      while (weekStart <= endDate) {
        ...
        for (const b of brushes) {
          // Skip entirely if this specific brush wasn't even in service
          // yet during this week — no expectation of logging something
          // that didn't exist. Matches the same principle as RO's own
          // activation-date gate.
          if (b.date_first_used && weekEndStr < b.date_first_used) continue;
  ...
  // Late means: submitting this right now would count as late for its
  // ISO week — same "first inspection after Wednesday 2:30pm" rule
  // _loadMissedEntries already applies retroactively, just checked
  // live instead. Passed to LateEntryReason as a function (not a
  // simple "HH:MM" string) since it also depends on which day of the
  // week it currently is, not just the time.
  _isLateNow() {
    const now = TrueTime.now();
    const weekStartStr = this._isoWeekStart(now);
    const wednesday = new Date(weekStartStr + 'T00:00:00');
    wednesday.setDate(wednesday.getDate() + 2); // Monday + 2 = Wednesday
    const wednesdayStr = UI.dateToStr(wednesday);
    const todayStr = UI.todayStr();
    if (todayStr > wednesdayStr) return true;
    if (todayStr < wednesdayStr) return false;
    const nowHHMM = `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`;
    return nowHHMM > '14:30';
  },
  ...
  _brushCard(b, loggedToday, loggedThisWeek) {
    ...
    return `
      <div class="card card-pad" data-brush-card="${UI.escapeHtml(b.brush_id)}" style="display:flex;flex-direction:column;gap:10px;">
        ...
        ${LateEntryReason.fieldHtml(`brush-late-${b.brush_id}`)}
  ...
  _bindCard(b) {
    ...
    LateEntryReason.wire(`brush-late-${b.brush_id}`, () => this._isLateNow());
    ...
    card.querySelector('[data-save]').addEventListener('click', async (e) => {
      const btn = e.target;
      const lateErr = LateEntryReason.validateOnSubmit(`brush-late-${b.brush_id}`, () => this._isLateNow());
      ...
      await UI.withLoading(btn, async () => {
        try {
          ...
          const who = await IdentityConfirm.confirm('Save Brush Entry');
          if (!who) return;
          ...
          const r2 = await DB.addBrushLog({
            ...
            late_reason: LateEntryReason.reasonForSave(`brush-late-${b.brush_id}`, () => this._isLateNow()),
            staff_id: who.id,
            staff_name: who.name
          });
