  ...
  async compute() {
    ...
    try {
      ...
      if (biMachines.length > 0 || dummyMachines.length > 0) {
        ...
        const weekStart = UI.dateToStr(this._isoWeekStart(new Date()));
    ...
    if (myLogbooks.includes('qa')) {
      ...
      machines.filter(m => m.qa_schedule_day).forEach(m => {
        ...
        dueDays.forEach(day => {
          ...
          const inWeek = (byMachine[m.machine_id] || []).filter(r => r.date_of_test >= day && r.date_of_test <= UI.dateToStr(weekEnd));
