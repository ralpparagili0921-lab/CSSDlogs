...
const App = {
  ...
  _setupMobileShell(role) {
    ...
    // 'categories' was a landing view removed in an earlier cleanup
    // pass (Aug 26) as an orphaned empty container — this reference
    // was missed at the time, and calling navigate('categories') ever
    // since has thrown (no #view-categories exists to un-hide) rather
    // than silently no-op, since navigate() doesn't null-check target.
    // 'cycles' is what the app already treats as home — App.init()
    // itself lands there by default (see navigate('cycles') above).
    document.getElementById('mobile-home-btn').addEventListener('click', () => this.navigate('cycles'));
  ...
  navigate(viewName) {
    ...
    const strip = document.getElementById('mobile-quickstrip');
    if (strip) strip.classList.toggle('on-home', viewName === 'cycles');
    const target = document.getElementById('view-' + viewName);
    if (!target) {
      // Defensive — a typo'd data-view attribute or a reference to a
      // since-removed view should never be able to throw and break
      // navigation entirely; the exact class of bug that caused the
      // 'categories' issue above. Fails loud enough to notice and
      // fix, without taking the whole app down.
      console.error(`navigate(): no view container for "${viewName}"`);
      return;
    }
